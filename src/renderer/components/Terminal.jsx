...
const TerminalPanel = ({ visible, onToggle, defaultCwd, }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribeData = ptyApi.onData((id, data) => {
            setSessions((prev) => {
        ...
        // Handle PTY exit
        const unsubscribeExit = ptyApi.onExit((id, _exitCode, _signal) => {
            setSessions((prev) => {
                const session = prev.find((s) => s.id === id);
                if (session?.terminal) {
    ...
    useEffect(() => {
        if (!visible || !activeSessionId || !ptyApi)
            return;
        setSessions((prev) => {
            ...
            return prev.map((s) => s.id === activeSessionId ? { ...s, terminal: term, fitAddon } : s);
        });
        return () => {
            if (resizeObserverRef.current) {
                resizeObserverRef.current.disconnect();
    ...
    if (!visible)
        return null;
    return (<div ref={containerRef} className="flex flex-col h-80 bg-neutral-950 border-t border-neutral-800 overflow-hidden">
      {/* Header with tab bar */}
      <div className="flex items-center bg-neutral-900 border-b border-neutral-800">
        ...
        <div className="flex items-center gap-0 overflow-x-auto flex-1 min-w-0">
          {sessions.map((session) => (<button key={session.id} onClick={() => setActiveSessionId(session.id)} className={`flex items-center gap-2 px-3 py-2 text-sm whitespace-nowrap border-r border-neutral-800 transition-colors ${activeSessionId === session.id
        ...
        <div className="flex items-center gap-1 px-2 py-2 border-l border-neutral-800">
          <button onClick={createSession} className="p-1.5 rounded hover:bg-neutral-800 text-neutral-400 hover:text-white transition-colors" title="New terminal">
            <Plus size={16}/>
          </button>
          <button onClick={onToggle} className="p-1.5 rounded hover:bg-neutral-800 text-neutral-400 hover:text-white transition-colors" title="Toggle terminal panel">
            <ChevronDown size={16}/>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden">
        {sessions.map((session) => (<div key={session.id} ref={(el) => {
                if (el) {
                    terminalRefs.current.set(session.id, el);
                }
            }} className={`w-full h-full bg-neutral-950 ${activeSessionId === session.id ? 'block' : 'hidden'}`}/>))}
        {sessions.length === 0 && (<div className="w-full h-full flex items-center justify-center text-neutral-500">
            <button onClick={createSession} className="px-4 py-2 rounded bg-neutral-800 hover:bg-neutral-700 transition-colors">
              + New Terminal
            </button>
          </div>)}
      </div>
