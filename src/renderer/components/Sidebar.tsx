...
export const Sidebar: React.FC<Props> = ({
  ...
  return (
    <div className="w-60 flex flex-col h-full bg-black/30 border-r border-white/5 flex-shrink-0">
      {/* New chat */}
      <div className="p-3">
        <button
          onClick={onNewChat}
          className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/70 hover:text-white text-sm transition-colors"
        >
          <Plus size={15} />
          <span>New chat</span>
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-2 space-y-4 pb-2">
          ...
          <div key={label}>
            ...
            {items.map((s) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-left text-xs truncate transition-colors ${
        ...
        {filtered.length === 0 && (
          <p className="text-white/20 text-xs text-center pt-8">No chats yet</p>
        )}
      </div>
      ...
      <div className="border-t border-white/5 p-2 flex items-center gap-1">
        <button
          ...
          className="flex items-center gap-1.5 px-2 py-1.5 rounded-md text-white/40 hover:text-white hover:bg-white/5 text-xs transition-colors"
        >
          <Settings size={13} />
          <span>Settings</span>
        </button>
        <button
          onClick={onOpenSettings}
          className="ml-auto flex items-center gap-1.5 px-2 py-1.5 rounded-md text-white/40 hover:text-white hover:bg-white/5 text-xs transition-colors"
        >
          <Plug size={13} />
          <span>MCP</span>
        </button>
