...
export const BootSplash: React.FC<Props> = ({ status, log, onRetry }) => {
  const [logs, setLogs] = useState<string[]>([])

  useEffect(() => {
    window.nyra.openclaw.onInstallLog?.((line: string) => {
      setLogs(prev => [...prev.slice(-8), line.trim()])
    })
    window.nyra.openclaw.onLog?.((line: string) => {
      setLogs(prev => [...prev.slice(-8), line.trim()])
    })
  }, [])
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-[#0c0c0c] text-white overflow-hidden">
      <TitleBar title="Nyra" />
      <div className="flex flex-col items-center justify-center flex-1 gap-6 px-12 text-center">
        ...
        {/* Live log output */}
        {logs.length > 0 && (
          <div className="w-full max-w-md bg-black/40 border border-white/5 rounded-xl p-3 font-mono text-[11px] text-left space-y-0.5">
            {logs.map((line, i) => (
              <p key={i} className={`truncate ${i === logs.length - 1 ? 'text-white/50' : 'text-white/25'}`}>
                {line}
              </p>
            ))}
          </div>
        ...
        {isError && (
          <button
            onClick={onRetry ?? (() => window.nyra.openclaw.restart())}
            className="flex items-center gap-2 px-5 py-2 bg-terra-500 hover:bg-terra-400 text-white text-sm font-medium rounded-xl transition-colors"
          >
            <RefreshCw size={13} />
            Retry
          </button>
