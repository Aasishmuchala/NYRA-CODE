...
export const ArtifactPane: React.FC<Props> = ({ artifacts, onClose }) => {
  ...
  return (
    <div className="w-[400px] flex-shrink-0 h-full bg-[#0e0e0e] border-l border-white/[0.07] flex flex-col overflow-hidden">
            ...
            {artifacts.map((a, i) => (
              <button
                ...
                className={`px-2 py-0.5 rounded text-[10px] font-mono transition-colors ${
                  i === activeIdx ? 'bg-terra-400/20 text-terra-300' : 'text-white/30 hover:text-white/60'
                }`}
              >
                {i + 1}
              </button>
        ...
        <div className="flex border-b border-white/[0.06] flex-shrink-0">
          {(['code', 'preview'] as Tab[]).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`flex items-center gap-1.5 px-4 py-2 text-[11px] font-medium transition-colors capitalize ${
                tab === t
      ...
      <div className="flex-1 overflow-hidden">
        {tab === 'code' ? (
          <CodeView code={artifact.code} lang={artifact.lang} />
        ) : (
          <PreviewView code={artifact.code} lang={artifact.lang} />
        )}
      </div>
...
const PreviewView: React.FC<{ code: string; lang: string }> = ({ code, lang }) => {
