...
import { ArrowRight, ArrowLeft, Check, Loader2, ExternalLink, Monitor, Cpu, Shield, Sparkles, Key, Zap, ChevronRight, HardDrive, Eye, } from 'lucide-react';
const STEPS = ['welcome', 'provider', 'ollama', 'capabilities', 'ready'];
// ── Main Component ─────────────────────────────────────────────────────────────
export const Onboarding = ({ onComplete }) => {
    ...
    const handleOAuth = async (providerId) => {
        ...
        try {
            ...
            if (providerId === 'copilot') {
                ...
                result = await window.nyra.providers.startOAuth(providerId);
            }
            // Always refresh provider states after OAuth resolves (belt + suspenders
            // with the onOAuthComplete listener — that event can race or get dropped
            // if the window was backgrounded during the browser flow)
            setOauthLoading(false);
            setStates(await window.nyra.providers.list());
    ...
    return (<div className="fixed inset-0 z-[100] bg-[#0b0a08] flex items-center justify-center">
      {/* Background gradient */}
      ...
      <div className={`relative w-full max-w-lg mx-4 transition-opacity duration-150 ${animating ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {STEPS.map((s, i) => (<div key={s} className={`transition-all duration-300 rounded-full ${i === stepIndex
        ...
        {step === 'welcome' && (<div className="text-center space-y-6">
            ...
            ].map(f => (<div key={f.label} className="flex items-start gap-2.5 bg-white/[0.03] border border-white/[0.06] rounded-xl px-3 py-2.5 text-left">
            ...
            <button onClick={goNext} className="inline-flex items-center gap-2 px-6 py-3 bg-terra-400 hover:bg-terra-500 text-white rounded-xl font-medium text-sm transition-colors shadow-lg shadow-terra-500/20 mt-2">
              Get Started <ArrowRight size={16}/>
            </button>
          </div>)}

        {/* ── Step 2: Provider Setup ──────────────────────────────────────── */}
        {step === 'provider' && (<div className="space-y-5">
            ...
            <div className="space-y-2">
              {catalog.map(def => {
                ...
                return (<button key={def.id} onClick={() => setSelectedProvider(isSelected ? null : def.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left ${isConnected
                    ...
                    {isConnected ? (<Check size={16} className="text-sage-400"/>) : (<ChevronRight size={14} className={`text-white/20 transition-transform ${isSelected ? 'rotate-90' : ''}`}/>)}
                  </button>);
            })}
            </div>

            {/* Expanded auth for selected provider */}
            {selectedProvider && !states.find(s => s.id === selectedProvider)?.hasKey && (<div className="bg-white/[0.02] border border-white/[0.08] rounded-xl p-4 space-y-3">
                {/* OAuth sign-in button for providers that support it */}
                {oauthAvail[selectedProvider] && (<>
                    <button onClick={() => handleOAuth(selectedProvider)} disabled={oauthLoading} className="w-full flex items-center justify-center gap-2 px-3 py-2.5 bg-terra-500/80 hover:bg-terra-400 disabled:bg-terra-500/40 text-white text-xs rounded-xl transition-colors font-medium">
                      {oauthLoading
                        ? <><Loader2 size={12} className="animate-spin"/> Waiting for sign-in...</>
                        : <><ExternalLink size={12}/> {selectedProvider === 'openai' ? 'Sign in with ChatGPT' :
                                selectedProvider === 'gemini' ? 'Sign in with Google' :
                                    selectedProvider === 'copilot' ? 'Sign in with GitHub' :
                                        `Sign in with ${catalog.find(c => c.id === selectedProvider)?.label ?? selectedProvider}`}</>}
                    </button>
                ...
                {/* "Get API key" link for providers without OAuth (like Anthropic) */}
                {!oauthAvail[selectedProvider] && catalog.find(c => c.id === selectedProvider)?.oauthUrl && (<>
                    <button onClick={() => window.nyra.providers.openOauth(catalog.find(c => c.id === selectedProvider).oauthUrl)} className="w-full flex items-center justify-center gap-2 px-3 py-2.5 bg-terra-500/80 hover:bg-terra-400 text-white text-xs rounded-xl transition-colors font-medium">
                      <ExternalLink size={12}/> Get {catalog.find(c => c.id === selectedProvider)?.label} API key
                    </button>
                    <p className="text-[10px] text-white/30 text-center">Paste your key below after copying it</p>
                  </>)}

                <div className="flex gap-2">
                  <input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSaveKey()} placeholder={catalog.find(c => c.id === selectedProvider)?.apiKeyPrefix
                    ? `${catalog.find(c => c.id === selectedProvider).apiKeyPrefix}...`
                    : 'Paste your API key'} className="flex-1 bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-white/80 placeholder-white/20 outline-none focus:border-terra-400/50 font-mono"/>
                  <button onClick={handleSaveKey} disabled={!apiKey.trim() || savingKey} className="px-3 py-2 bg-sage-600/80 hover:bg-sage-500 disabled:bg-white/[0.05] disabled:text-white/20 text-white text-xs rounded-lg transition-colors font-medium flex items-center gap-1.5">
            ...
            {/* Error banner */}
            {error && (<div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2 text-xs text-red-300">
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <button onClick={goNext} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-colors ${hasConnectedProvider
        ...
        {step === 'ollama' && (<div className="space-y-5">
            <div className="text-center space-y-2">
            ...
            <div className={`flex items-center gap-3 border rounded-xl px-4 py-3 ${ollamaOnline ? 'border-sage-500/25 bg-sage-500/[0.03]' : 'border-white/[0.07] bg-white/[0.02]'}`}>
              <span className={`w-3 h-3 rounded-full flex-shrink-0 ${ollamaChecking ? 'bg-gold-400 animate-pulse' : ollamaOnline ? 'bg-sage-400' : 'bg-red-500'}`}/>
              <div className="flex-1">
                <p className="text-sm text-white font-medium">
                  {ollamaChecking ? 'Checking for Ollama…' : ollamaOnline ? 'Ollama Detected' : 'Ollama Not Found'}
                </p>
                {ollamaOnline && ollamaModels.length > 0 && (<p className="text-[10px] text-white/30 mt-0.5">
                ...
                {!ollamaOnline && !ollamaChecking && (<p className="text-[10px] text-white/30 mt-0.5">
                    Install Ollama to run models like Llama 3, Mistral, and Phi-4 locally
                  </p>)}
              </div>
              {!ollamaOnline && !ollamaChecking && (<button onClick={() => window.nyra.app.openExternal('https://ollama.com')} className="px-3 py-2 bg-terra-400 hover:bg-terra-500 text-white text-xs rounded-lg transition-colors font-medium flex items-center gap-1.5 flex-shrink-0">
                  <ExternalLink size={11}/> Get Ollama
                </button>)}
            </div>

            {ollamaOnline && (<div className="bg-white/[0.02] border border-white/[0.07] rounded-xl p-4 space-y-2">
                ...
                ].map(m => (<div key={m.name} className="flex items-center gap-2 border border-white/[0.06] rounded-lg px-3 py-2">
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <button onClick={goNext} className="flex items-center gap-2 px-5 py-2.5 bg-terra-400 hover:bg-terra-500 text-white rounded-xl text-sm font-medium transition-colors">
        ...
        {/* ── Step 4: Capabilities ────────────────────────────────────────── */}
        {step === 'capabilities' && (<div className="space-y-5">
            <div className="text-center space-y-2">
              ...
              <div className="border border-white/[0.07] bg-white/[0.02] rounded-xl px-4 py-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <button onClick={handleTestScreen} className={`mt-2 flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg transition-colors font-medium ${screenTested && screenWorking
                ? 'bg-sage-500/15 text-sage-400 border border-sage-500/20'
                : 'bg-white/[0.06] hover:bg-white/[0.1] text-white/60 hover:text-white/80'}`}>
                      {screenTested && screenWorking ? (<><Check size={12}/> Screen capture working</>) : screenTested ? (<><Monitor size={12}/> Permission needed — check System Settings</>) : (<><Monitor size={12}/> Test screen capture</>)}
                    </button>
                  </div>
                </div>
              </div>
        ...
        {/* ── Step 5: Ready ───────────────────────────────────────────────── */}
        {step === 'ready' && (<div className="text-center space-y-6">
            ...
            ].map(t => (<div key={t.keys} className="flex items-center gap-3 bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2">
            ...
            <button onClick={onComplete} className="inline-flex items-center gap-2 px-8 py-3 bg-terra-400 hover:bg-terra-500 text-white rounded-xl font-medium text-sm transition-colors shadow-lg shadow-terra-500/20 mt-2">
