...
import {
  ...
  Palette, Server, Shield, DollarSign, Megaphone, Scale, GraduationCap, HeartPulse, FlaskConical, MessageSquare,
...
export const SkillsMarketplace: React.FC<Props> = ({ onClose: _onClose }) => {
  ...
  useEffect(() => {
    const loadSkills = async () => {
      try {
        ...
        setAllSkills(browsed as MarketplaceSkill[])
        const inst = await window.nyra.skills.installed()
        setInstalled(inst as MarketplaceSkill[])
      } catch (err) {
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < filled) {
        stars.push(<Star key={i} className="w-3 h-3 fill-gold-400 text-gold-400" />)
      } else {
        stars.push(<Star key={i} className="w-3 h-3 text-gold-400/25" />)
      }
    }
    return stars
  }
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      {tab === 'browse' && (
        <div className="flex flex-col flex-1 min-h-0">
          ...
          <div className="flex-shrink-0 mb-3">
            <div className="flex items-center gap-2 bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2">
              ...
              <input
                ...
                className="flex-1 bg-transparent outline-none text-white text-xs placeholder-white/40"
              />
              {search && (
          ...
          <div className="flex flex-wrap gap-1.5 mb-3 flex-shrink-0">
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-2 py-1 rounded-md text-[11px] transition-colors flex items-center gap-1 capitalize ${
          ...
          {/* Error */}
          {error && (
            <div className="flex-shrink-0 text-xs text-blush-300 bg-blush-400/5 rounded-lg px-3 py-2 mb-2">
              {error}
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            <div className="space-y-2">
              {paged.map(skill => (
                <div
                  key={skill.id}
                  className="bg-white/[0.02] border border-white/[0.06] rounded-lg p-3 hover:bg-white/[0.04] transition-colors flex items-start gap-3"
                >
                  ...
                  <div className="flex-shrink-0">
                    {skill.installedLocally ? (
                      <button
                        ...
                        className="px-2.5 py-1.5 text-[11px] rounded-lg bg-blush-400/10 hover:bg-blush-400/20 text-blush-300 transition-colors flex items-center gap-1 disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 text-[11px] rounded-lg bg-terra-500/25 hover:bg-terra-500/35 text-terra-200 transition-colors flex items-center gap-1 disabled:opacity-50"
                      >
              ...
              {filtered.length === 0 && (
                <div className="text-center py-12 text-white/30 text-xs">No skills match your search.</div>
              )}
            </div>
          </div>
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-3 flex-shrink-0 border-t border-white/[0.06] mt-2">
              <button
                onClick={() => setPage(p => Math.max(0, p - 1))}
                disabled={page === 0}
                className="flex items-center gap-1 px-2 py-1 text-[11px] text-white/40 hover:text-white/70 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
          ...
          {installed.length === 0 ? (
            ...
            <div className="space-y-2">
              {installed.map(skill => (
                <div
                  key={skill.id}
                  className="bg-white/[0.02] border border-white/[0.06] rounded-lg p-3 flex items-center gap-3 hover:bg-white/[0.04] transition-colors"
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      className={`px-2.5 py-1.5 text-[11px] rounded-lg transition-colors flex items-center gap-1 ${
                        skill.enabled
                          ? 'bg-sage-400/10 hover:bg-sage-400/20 text-sage-300'
                    ...
                    <button
                      onClick={() => handleRemove(skill.id)}
                      disabled={removing === skill.id}
                      className="px-2 py-1.5 text-[11px] rounded-lg bg-blush-400/10 hover:bg-blush-400/20 text-blush-300 transition-colors disabled:opacity-50"
                    >
                      {removing === skill.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <X className="w-3 h-3" />}
                    </button>
                  </div>
