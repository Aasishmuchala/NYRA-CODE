...
export type ActionResponse = 'approved' | 'denied' | 'timeout'

interface ActionQueueContextType {
  pendingAction: DesktopAction | null
  enqueue: (action: DesktopAction) => void
  approve: (actionId: string) => void
  deny: (actionId: string) => void
  alwaysAllow: (actionType: string) => void
  alwaysAllowList: Set<string>
}
...
export function ActionQueueProvider({ children }: { children: React.ReactNode }) {
  const [queue, setQueue] = useState<DesktopAction[]>([])
  const [alwaysAllowList, setAlwaysAllowList] = useState<Set<string>>(new Set())
  const pendingCallbacks = useRef<
    Map<string, { resolve: (response: ActionResponse) => void }>
  >(new Map())
  ...
  const deny = useCallback((actionId: string) => {
    const callback = pendingCallbacks.current.get(actionId)
    if (callback) {
  ...
  const alwaysAllow = useCallback((actionType: string) => {
...
// Action icon mapping
const ACTION_ICONS: Record<DesktopAction['type'], React.ElementType> = {
  'mouse-click': Mouse,
  'mouse-move': Mouse,
...
// Action type labels
const ACTION_LABELS: Record<DesktopAction['type'], string> = {
          ...
          <div className="px-6 py-4 bg-gradient-to-b from-[#0c0c0c] to-[#050505] space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-2 bg-terra-400/10 hover:bg-terra-400/20 border border-terra-400/30 hover:border-terra-400/50 rounded-md text-terra-400 hover:text-terra-300 font-medium text-sm transition-colors duration-150"
              >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-sage-500/10 hover:bg-sage-500/20 border border-sage-500/50 hover:border-sage-500/70 rounded-md text-sage-400 hover:text-sage-400 font-medium text-sm transition-colors duration-150"
            >
