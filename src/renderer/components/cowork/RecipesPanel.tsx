...
import {
  BookTemplate, Play, Trash2, ChevronRight, ChevronDown,
  Loader2, Check, X, Download, RefreshCw, AlertTriangle,
} from 'lucide-react'

// ── Category Badge ────────────────────────────────────────────────────────────

const CATEGORY_COLORS: Record<string, string> = {
  development: 'bg-terra-400/10 text-terra-300',
  deployment: 'bg-sage-400/10 text-sage-300',
  review: 'bg-gold-400/10 text-gold-300',
  data: 'bg-gold-400/10 text-gold-300',
  custom: 'bg-white/[0.06] text-white/50',
}

const CategoryBadge: React.FC<{ category: string }> = ({ category }) => (
  <span className={`text-[9px] px-1.5 py-0.5 rounded ${CATEGORY_COLORS[category] || CATEGORY_COLORS.custom}`}>
    {category}
  </span>
)
...
const StepStatusIcon: React.FC<{ status?: string }> = ({ status }) => {
  switch (status) {
    case 'completed': return <Check size={10} className="text-sage-300" />
    case 'running':   return <Loader2 size={10} className="text-terra-300 animate-spin" />
    case 'failed':    return <AlertTriangle size={10} className="text-blush-300" />
...
const RecipeCard: React.FC<{
  ...
  onDelete: () => void
  onExport: () => void
}> = ({ recipe, expanded, activeRun, onToggle, onRun, onDelete, onExport }) => (
      ...
      <div className="border-t border-white/[0.06] px-3 py-2 bg-black/20 space-y-2">
        ...
        <div className="space-y-1">
          ...
          {recipe.steps?.map((step: any, i: number) => (
            <div key={step.id} className="flex items-center gap-2 px-2 py-1 rounded bg-white/[0.02]">
        ...
        {recipe.variables && Object.keys(recipe.variables).length > 0 && (
          <div className="space-y-1">
            ...
            {Object.entries(recipe.variables).map(([key, val]) => (
        ...
        <div className="flex items-center gap-1.5 pt-1">
          <button onClick={onRun}
            className="flex items-center gap-1 px-2 py-1 rounded bg-sage-400/10 text-sage-300 text-[9px] hover:bg-sage-400/20">
            <Play size={10} /> Run
          </button>
          <button onClick={onExport}
            className="flex items-center gap-1 px-2 py-1 rounded bg-terra-400/10 text-terra-300 text-[9px] hover:bg-terra-400/20">
            <Download size={10} /> Export
          </button>
          {!recipe.builtin && (
            <button onClick={onDelete}
              className="flex items-center gap-1 px-2 py-1 rounded bg-blush-400/10 text-blush-300 text-[9px] hover:bg-blush-400/20">
        ...
        {activeRun && (
          <div className="border-t border-white/[0.06] pt-2 mt-2">
            <p className="text-[9px] text-white/40">
              Run: <span className={activeRun.status === 'completed' ? 'text-sage-300' : activeRun.status === 'failed' ? 'text-blush-300' : 'text-terra-300'}>
                {activeRun.status}
              </span>
              {activeRun.error && <span className="text-blush-300/60 ml-2">{activeRun.error}</span>}
            </p>
          </div>
        )}
      </div>
...
const RecipesPanel: React.FC = () => {
  ...
  const [activeRuns, setActiveRuns] = useState<Map<string, any>>(new Map())
  const [view, setView] = useState<'recipes' | 'runs'>('recipes')
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-white/[0.06]">
        <button onClick={() => setView('recipes')}
          className={`px-2 py-1 rounded text-[10px] ${view === 'recipes' ? 'bg-terra-300/10 text-terra-300' : 'text-white/50 hover:text-white/70'}`}>
          Recipes
        </button>
        <button onClick={() => setView('runs')}
          className={`px-2 py-1 rounded text-[10px] ${view === 'runs' ? 'bg-terra-300/10 text-terra-300' : 'text-white/50 hover:text-white/70'}`}>
          Runs ({runs.length})
        </button>
      ...
      {/* Category filter */}
      {view === 'recipes' && categories.length > 1 && (
        <div className="flex items-center gap-1 px-4 py-1.5 border-b border-white/[0.03] overflow-x-auto scrollbar-thin">
          <button onClick={() => setSelectedCategory(null)}
            className={`px-2 py-0.5 rounded text-[9px] whitespace-nowrap ${!selectedCategory ? 'bg-terra-300/10 text-terra-300' : 'text-white/40'}`}>
            All
          </button>
          {categories.map(cat => (
            <button key={cat} onClick={() => setSelectedCategory(cat)}
              className={`px-2 py-0.5 rounded text-[9px] whitespace-nowrap ${selectedCategory === cat ? 'bg-terra-300/10 text-terra-300' : 'text-white/40'}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin p-4 space-y-2">
          ...
          <>
            ...
            {recipes.map(recipe => (
              <RecipeCard
                ...
                activeRun={activeRuns.get(recipe.id)}
                onToggle={() => toggleExpand(recipe.id)}
                onRun={() => handleRun(recipe.id)}
                onDelete={() => handleDelete(recipe.id)}
                onExport={() => handleExport(recipe.id)}
              />
            ))}
          </>
        )}
        ...
        {view === 'runs' && (
          <>
            ...
            {runs.map(run => (
              <div key={run.id} className="border border-white/[0.06] rounded-lg p-2 space-y-1">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] ${
                    run.status === 'completed' ? 'text-sage-300' :
                    run.status === 'failed' ? 'text-blush-300' :
                    run.status === 'running' ? 'text-terra-300' : 'text-white/40'
                  }`}>{run.status}</span>
                ...
                {run.error && <p className="text-[9px] text-blush-300/60 truncate">{run.error}</p>}
              </div>
            ))}
          </>
        )}
