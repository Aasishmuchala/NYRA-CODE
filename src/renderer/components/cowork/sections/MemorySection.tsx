...
type MemoryType = 'fact' | 'preference' | 'decision' | 'code_pattern' | 'context' | 'summary'
...
const TYPE_COLORS: Record<MemoryType, string> = {
  fact:         'text-terra-400/70',
  preference:   'text-gold-400/70',
  decision:     'text-gold-400/70',
  code_pattern: 'text-sage-400/70',
  context:      'text-terra-400/70',
  summary:      'text-blush-400/70',
}
    ...
    <div className="space-y-2">
      ...
      {stats && (
        <div className="flex items-center gap-3 text-[9px] text-white/45">
          ...
          <span className={stats.embeddingModelAvailable ? 'text-sage-400/50' : 'text-gold-400/50'}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white/[0.04] border border-white/[0.07] rounded-md pl-6 pr-2 py-1.5 text-[10px] text-white/70 placeholder:text-white/40 outline-none focus:border-terra-300/30"
        />
      </div>
          ...
          {memories.map(entry => (
            <div key={entry.id} className="flex items-start gap-2 py-1 group">
              ...
              <div className="flex-1 min-w-0">
                ...
                {entry.topic && (
                  <span className="text-[8px] text-white/40 mt-0.5 block">{entry.topic}</span>
                )}
              </div>
              {entry.pinned && <Pin size={9} className="text-gold-400/50 flex-shrink-0 mt-0.5" />}
            </div>
          ))}
        </div>
      ) : (
