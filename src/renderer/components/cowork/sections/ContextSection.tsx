...
import {
  File, Folder, Clipboard, Globe, Database, Pin,
} from 'lucide-react'
...
const ContextSection: React.FC = () => {
  ...
  const barColor = usagePct < 33 ? 'bg-sage-500' : usagePct < 66 ? 'bg-gold-500' : 'bg-blush-500'
  ...
  return (
    <div className="space-y-3">
      ...
      {sources.length > 0 ? (
          ...
          {sources.filter(s => s.active).map(source => {
            ...
            return (
              <div key={source.id} className="flex items-center gap-2 py-1">
                ...
                {source.pinned && <Pin size={9} className="text-gold-400/60 flex-shrink-0" />}
        ...
        <p className="text-[10px] text-white/30 py-1">No context sources</p>
      )}
