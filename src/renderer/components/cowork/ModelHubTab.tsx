...
import {
  Download, Trash2, Search, Cpu, HardDrive, Play, Pause,
  X, ChevronRight, Zap, RefreshCw, CheckCircle,
  AlertTriangle, Loader2,
} from 'lucide-react'

// ── Sub-components ──────────────────────────────────────────────────────────

const VramBar: React.FC<{ usedMb: number; totalMb: number }> = ({ usedMb, totalMb }) => {
  const pct = totalMb > 0 ? Math.min(100, (usedMb / totalMb) * 100) : 0
  const color = pct > 90 ? 'bg-blush-400' : pct > 70 ? 'bg-gold-400' : 'bg-sage-400'
  return (
...
const CapBadge: React.FC<{ cap: string }> = ({ cap }) => {
  const colors: Record<string, string> = {
    chat: 'bg-terra-400/20 text-terra-300',
    code: 'bg-sage-400/20 text-sage-300',
    vision: 'bg-gold-400/20 text-gold-300',
    tools: 'bg-gold-400/15 text-gold-300',
    embeddings: 'bg-terra-400/15 text-terra-300',
  }
  return (
    <span className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${colors[cap] || 'bg-white/10 text-white/50'}`}>
      {cap}
    </span>
  )
}

const DownloadProgress: React.FC<{ job: any }> = ({ job }) => {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-white/[0.03] rounded-lg border border-white/[0.06]">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${
            job.status === 'completed' ? 'bg-sage-400/20 text-sage-300' :
            job.status === 'failed' ? 'bg-blush-400/20 text-blush-300' :
            job.status === 'paused' ? 'bg-gold-400/20 text-gold-300' :
            'bg-terra-400/20 text-terra-300'
          }`}>{job.status}</span>
        </div>
        {isActive && (
      ...
      <div className="flex items-center gap-1">
        {isActive && (
          <button onClick={() => window.nyra.modelHub.pauseDownload(job.id)} className="p-1 hover:bg-white/10 rounded" title="Pause">
            <Pause size={12} className="text-white/50" />
          </button>
        )}
        {job.status === 'paused' && (
          <button onClick={() => window.nyra.modelHub.resumeDownload(job.id)} className="p-1 hover:bg-white/10 rounded" title="Resume">
            <Play size={12} className="text-white/50" />
          </button>
        )}
        {(isActive || job.status === 'paused' || job.status === 'queued') && (
          <button onClick={() => window.nyra.modelHub.cancelDownload(job.id)} className="p-1 hover:bg-white/10 rounded" title="Cancel">
...
const ModelCardView: React.FC<{ card: any; onClose: () => void }> = ({ card, onClose }) => {
  const [fitCheck, setFitCheck] = useState<{ fits: boolean; requiredMb: number; availableMb: number } | null>(null)
  ...
  return (
    <div className="p-4 bg-white/[0.03] rounded-xl border border-white/[0.08] space-y-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        {card.capabilities?.map((c: string) => <CapBadge key={c} cap={c} />)}
      </div>
      ...
      {fitCheck && (
        <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[10px] ${fitCheck.fits ? 'bg-sage-400/20 text-sage-300' : 'bg-gold-400/20 text-gold-300'}`}>
      ...
      <div className="flex gap-2">
        {card.installed ? (
          <button
            onClick={() => window.nyra.modelHub.removeModel(card.name)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blush-400/20 text-blush-300 text-[10px] hover:bg-blush-400/30 transition"
          >
            <Trash2 size={12} /> Remove
          </button>
        ) : (
          <button
            onClick={() => window.nyra.modelHub.startDownload(card.name)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-terra-300/10 text-terra-300 text-[10px] hover:bg-terra-300/20 transition"
          >
...
const ComparePanel: React.FC<{ models: string[]; onClose: () => void }> = ({ models, onClose }) => {
    ...
    <div className="p-4 bg-white/[0.03] rounded-xl border border-white/[0.08] space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <select value={modelA} onChange={e => setModelA(e.target.value)}
          className="bg-white/[0.06] border border-white/[0.08] rounded-lg px-2 py-1.5 text-[10px] text-white/80 outline-none">
      ...
      {result && (
        <div className="grid grid-cols-2 gap-2">
          {[result.modelA, result.modelB].map((r: any) => (
            <div key={r.name} className="bg-white/[0.04] rounded-lg p-3 space-y-1">
...
type ViewMode = 'installed' | 'library' | 'downloads' | 'compare'

const ModelHubTab: React.FC = () => {
  ...
  useEffect(() => {
    const unsubs = [
      window.nyra.modelHub.onDownloadProgress((d: any) => {
        setDownloads(prev => prev.map(j => j.id === d.id ? d : j))
      }),
      window.nyra.modelHub.onDownloadCompleted((d: any) => {
        setDownloads(prev => prev.map(j => j.id === d.id ? d : j))
        refresh() // Refresh installed list
      }),
      window.nyra.modelHub.onDownloadFailed((d: any) => {
        setDownloads(prev => prev.map(j => j.id === d.id ? d : j))
      }),
  ...
  if (!ollamaOnline && !loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-white/40 text-center p-8">
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b border-white/[0.06] space-y-2">
        ...
        <div className="flex items-center gap-1.5">
          {(['installed', 'library', 'downloads', 'compare'] as ViewMode[]).map(m => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className={`px-2.5 py-1 rounded-full text-[10px] font-medium transition ${
                mode === m ? 'bg-terra-300/20 text-terra-300' : 'text-white/50 hover:text-white/70 hover:bg-white/5'
              }`}
            >
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin p-4 space-y-3">
        ...
        {/* ── Selected Model Card ──────────────────────────────────────── */}
        {selectedCard && (
          <ModelCardView card={selectedCard} onClose={() => setSelectedCard(null)} />
        )}
          ...
          <>
            {installed.length === 0 ? (
              ...
              installed.map(m => (
                <button
                  key={m.id}
                  onClick={() => openCard(m.name)}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] transition text-left"
                >
        ...
        {mode === 'library' && !selectedCard && (
          <>
            <div className="flex gap-2">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full bg-white/[0.06] border border-white/[0.08] rounded-lg pl-7 pr-3 py-1.5 text-[10px] text-white/80 placeholder:text-white/30 outline-none"
                />
              </div>
              <select
                value={familyFilter}
            ...
            {library.map(m => (
              <div key={m.name} className="px-3 py-2.5 rounded-lg bg-white/[0.03] border border-white/[0.06] space-y-1.5">
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  {m.tags.slice(0, 6).map((t: string) => (
                    <button
                      key={t}
                      onClick={() => openCard(`${m.name}:${t}`)}
                      className="px-1.5 py-0.5 rounded bg-white/[0.06] text-[9px] text-white/50 hover:text-white/70 hover:bg-white/10 transition"
                    >
                      {t}
                    </button>
                  ))}
        ...
        {mode === 'downloads' && (
          <>
            {downloads.length === 0 ? (
              <p className="text-center py-8 text-white/30 text-[11px]">No downloads yet</p>
            ) : (
              downloads.map(d => <DownloadProgress key={d.id} job={d} />)
            )}
          </>
        )}
        ...
        {mode === 'compare' && (
          <>
            {installed.length < 2 ? (
              <p className="text-center py-8 text-white/30 text-[11px]">Install at least 2 models to compare</p>
            ) : (
              <ComparePanel models={installed.map((m: any) => m.name)} onClose={() => setMode('installed')} />
            )}
          </>
        )}
      </div>
