...
import {
  Zap, Plus, Trash2, Play, Pause, ChevronRight, ChevronDown,
  AlertTriangle, Check, X, RefreshCw, Loader2, Activity,
} from 'lucide-react'

type ActionType = 'run-agent' | 'send-notification' | 'execute-command' | 'call-tool' | 'run-script'
...
const OPERATOR_OPTIONS = ['equals', 'contains', 'matches', 'startsWith', 'endsWith', 'exists'] as const
...
const StatsBar: React.FC<{ stats: any }> = ({ stats }) => (
  <div className="flex items-center gap-4 px-4 py-2 border-b border-white/[0.06] text-[9px] text-white/40">
...
const RuleRow: React.FC<{
  ...
  onTest: () => void
}> = ({ rule, expanded, onToggle, onDelete, onToggleEnabled, onTest }) => (
  <div className="border border-white/[0.06] rounded-lg overflow-hidden">
    <button onClick={onToggle} className="w-full flex items-center gap-2 px-3 py-2 hover:bg-white/[0.04] transition text-left">
      ...
      <Zap size={14} className={rule.enabled ? 'text-gold-400' : 'text-white/20'} />
      ...
      <div className="border-t border-white/[0.06] px-3 py-2 bg-black/20 space-y-2">
        ...
        <div className="flex flex-wrap gap-1">
          {rule.conditions?.map((c: any, i: number) => (
            <span key={i} className="text-[9px] px-1.5 py-0.5 rounded bg-terra-500/10 text-terra-300">
        ...
        <div className="flex flex-wrap gap-1">
          {rule.actions?.map((a: any, i: number) => (
            <span key={i} className="text-[9px] px-1.5 py-0.5 rounded bg-sage-500/10 text-sage-300">
        ...
        <div className="flex items-center gap-1.5 pt-1">
          <button onClick={onToggleEnabled}
            className={`flex items-center gap-1 px-2 py-1 rounded text-[9px] ${
              rule.enabled ? 'bg-gold-500/10 text-gold-300' : 'bg-white/[0.06] text-white/40'
            } hover:opacity-80`}>
            {rule.enabled ? <><Pause size={10} /> Disable</> : <><Play size={10} /> Enable</>}
          </button>
          <button onClick={onTest}
            className="flex items-center gap-1 px-2 py-1 rounded bg-terra-500/10 text-terra-300 text-[9px] hover:bg-terra-500/20">
            <Play size={10} /> Test
          </button>
          <button onClick={onDelete}
            className="flex items-center gap-1 px-2 py-1 rounded bg-blush-500/10 text-blush-300 text-[9px] hover:bg-blush-500/20">
...
const LogRow: React.FC<{ log: any }> = ({ log }) => (
  <div className="flex items-center gap-2 px-3 py-1.5 text-[10px] border-b border-white/[0.03]">
    {log.actionsFailed > 0
...
const AddRuleForm: React.FC<{ onAdd: (rule: any) => void; onCancel: () => void }> = ({ onAdd, onCancel }) => {
  const [name, setName] = useState('')
  const [event, setEvent] = useState<string>(EVENT_OPTIONS[0])
  const [condField, setCondField] = useState('')
  const [condOp, setCondOp] = useState<string>('contains')
  const [condValue, setCondValue] = useState('')
  const [actionType, setActionType] = useState<ActionType>('send-notification')
  ...
  return (
    <div className="border border-white/[0.08] rounded-lg p-3 bg-white/[0.02] space-y-2">
      ...
      <div className="flex gap-2 items-center">
        ...
        <select value={actionType} onChange={e => setActionType(e.target.value as ActionType)} className={selectCls}>
      ...
      <input value={actionTitle} onChange={e => setActionTitle(e.target.value)}
        placeholder={actionType === 'execute-command' ? 'Command...' : 'Title / Task name...'}
        className={inputCls} />
      ...
      <div className="flex gap-1.5 pt-1">
        <button onClick={handleSubmit}
          className="flex items-center gap-1 px-2 py-1 rounded bg-sage-500/10 text-sage-300 text-[9px] hover:bg-sage-500/20">
...
const AutomationsPanel: React.FC = () => {
  ...
  const [stats, setStats] = useState<any>({ totalRules: 0, enabledRules: 0, totalTriggers: 0, recentLogs: 0 })
  ...
  const handleAdd = async (opts: any) => {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-white/[0.06]">
        <button onClick={() => setView('rules')}
          className={`px-2 py-1 rounded text-[10px] ${view === 'rules' ? 'bg-terra-500/10 text-terra-300' : 'text-white/50 hover:text-white/70'}`}>
          Rules
        </button>
        <button onClick={() => setView('logs')}
          className={`px-2 py-1 rounded text-[10px] ${view === 'logs' ? 'bg-terra-500/10 text-terra-300' : 'text-white/50 hover:text-white/70'}`}>
          Logs
        </button>
