...
const getActionType = (action: string): string => {
  ...
  if (actionLower.includes('create')) return 'file_create';
  if (actionLower.includes('edit') || actionLower.includes('update')) return 'file_edit';
  if (actionLower.includes('delete')) return 'file_delete';
  if (actionLower.includes('command') || actionLower.includes('run')) return 'command_run';
  if (actionLower.includes('approval')) return 'approval_request';
  if (actionLower.includes('transition')) return 'task_transition';
  return 'file_edit';
};

const actionTypeColors: Record<string, string> = {
  file_create: 'bg-sage-400/20 text-sage-300 border-sage-400/30',
  file_edit: 'bg-terra-400/20 text-terra-300 border-terra-400/30',
  file_delete: 'bg-blush-400/20 text-blush-300 border-blush-400/30',
  command_run: 'bg-gold-400/20 text-gold-300 border-gold-400/30',
  approval_request: 'bg-gold-400/20 text-gold-300 border-gold-400/30',
...
export const AuditLog: React.FC = () => {
  ...
  const uniqueAgents = useMemo(
    () => Array.from(new Set(entries.map((entry) => entry.agentId))),
    [entries]
  );
  ...
  const handleFilterChange = async (agentId?: string, action?: string) => {
    try {
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            {loading ? (
              ...
              <span className="text-xs text-white/30">({filteredEntries.length} entries)</span>
            )}
          </div>
          <div className="flex gap-2">
            <button
              ...
              disabled={loading}
              className="px-3 py-1.5 rounded-lg bg-white/[0.05] hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed text-white/60 text-xs font-medium flex items-center gap-1.5 transition-colors"
            >
              <Download size={14} />
              JSON
            </button>
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 bg-white/[0.05] border border-white/[0.08] rounded-lg text-xs text-white/80 placeholder-white/25 focus:outline-none focus:border-white/15"
            />
          </div>
                    ...
                    <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-3 hover:border-white/[0.12] transition-colors">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex items-center gap-2 flex-1">
                          <div
                            className={`p-1.5 rounded border ${actionTypeColors[actionType] || actionTypeColors.file_edit}`}
                          >
                      ...
                      <div className="flex items-center gap-2 pl-10">
                        {entry.reversible && (
                          <button className="px-2.5 py-1 rounded border border-white/[0.08] bg-white/[0.04] hover:bg-white/[0.08] text-white/60 text-xs font-medium flex items-center gap-1.5 transition-colors">
                            <RotateCcw size={12} />
                            Rollback
                          </button>
                        )}
