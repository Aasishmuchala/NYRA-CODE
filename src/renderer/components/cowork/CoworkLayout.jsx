...
import { ChevronDown, ChevronRight, ListTodo, FolderOpen, Brain, Bot, Map, Database, Layers, Zap, BookOpen, Globe, BookTemplate, FileCode, Shield, Cpu, } from 'lucide-react';
...
const Section = ({ label, icon: Icon, badge, expanded, onToggle, children }) => (<div className="border-b border-white/[0.06] last:border-b-0">
    <button onClick={onToggle} className="w-full flex items-center gap-2.5 px-4 py-3 hover:bg-white/[0.03] transition-colors cursor-pointer group">
      ...
      {badge !== undefined && badge !== '' && (<span className="text-[10px] text-white/40 font-medium tabular-nums">{badge}</span>)}
      {expanded ? (<ChevronDown size={13} className="text-white/30 flex-shrink-0"/>) : (<ChevronRight size={13} className="text-white/30 flex-shrink-0"/>)}
    </button>
...
// ── Tool Toolbar Button ─────────────────────────────────────────────────────
const ToolButton = ({ label, icon: Icon, active, onClick }) => (<button onClick={onClick} className={`
...
const CoworkLayout = ({ badges = {} }) => {
    ...
    const tools = [
        ...
        { id: 'knowledge', label: 'RAG', icon: BookOpen },
    ...
    return (<div className="flex flex-col h-full bg-black/10">
      {/* ── Tool Toolbar ─────────────────────────────────────────────── */}
      <div className="flex items-center gap-1 px-3 py-2 border-b border-white/[0.06] bg-black/20 overflow-x-auto scrollbar-thin">
        {tools.map(tool => (<ToolButton key={tool.id} label={tool.label} icon={tool.icon} active={activeTool === tool.id} onClick={() => toggleTool(tool.id)}/>))}
      </div>
      ...
      {activeTool ? (
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          <Section id="progress" label="Progress" icon={ListTodo} badge={badges.progress} expanded={expanded.progress} onToggle={() => toggleSection('progress')}>
            <ProgressSection />
          </Section>

          <Section id="plan" label="Plan" icon={Map} badge={badges.plan} expanded={expanded.plan} onToggle={() => toggleSection('plan')}>
            <PlanSection />
          </Section>

          <Section id="folders" label="Working Folders" icon={FolderOpen} badge={badges.folders} expanded={expanded.folders} onToggle={() => toggleSection('folders')}>
            <FoldersSection />
          </Section>

          <Section id="context" label="Context" icon={Brain} badge={badges.context} expanded={expanded.context} onToggle={() => toggleSection('context')}>
            <ContextSection />
          </Section>

          <Section id="agents" label="Agents" icon={Bot} badge={badges.agents} expanded={expanded.agents} onToggle={() => toggleSection('agents')}>
            <AgentsSection />
          </Section>

          <Section id="memory" label="Memory" icon={Database} badge={badges.memory} expanded={expanded.memory} onToggle={() => toggleSection('memory')}>
            <MemorySection />
          </Section>
        </div>)}
    </div>);
