...
import {
  BookOpen, Trash2, Upload, Search, FileText,
  Loader2, X, Check, FolderPlus, Database,
} from 'lucide-react'
...
const StackCard: React.FC<{
  ...
  onSelect: () => void
  onDelete: () => void
}> = ({ stack, selected, onSelect, onDelete }) => (
  <button
    onClick={onSelect}
    className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg transition text-left ${
      selected ? 'bg-terra-300/10 border border-terra-300/30' : 'border border-white/[0.06] hover:bg-white/[0.04]'
    }`}
  >
    ...
    <button onClick={e => { e.stopPropagation(); onDelete() }}
      className="p-0.5 hover:bg-blush-400/20 rounded opacity-0 group-hover:opacity-100"
      title="Delete stack">
      <Trash2 size={10} className="text-blush-300" />
    </button>
  </button>
)
...
const DocRow: React.FC<{
  ...
  <div className="flex items-center gap-2 px-3 py-1.5 border-b border-white/[0.03]">
    ...
    <button onClick={onRemove} className="p-0.5 hover:bg-blush-400/20 rounded" title="Remove">
      <Trash2 size={10} className="text-blush-300/60" />
    </button>
  </div>
)
...
const SearchResult: React.FC<{ result: any; index: number }> = ({ result, index }) => (
  <div className="border border-white/[0.06] rounded-lg p-2 space-y-1">
    <div className="flex items-center gap-2">
      ...
      <span className="text-[9px] px-1.5 py-0.5 rounded bg-sage-400/20 text-sage-300">
...
const CreateStackForm: React.FC<{ onCreate: (name: string, desc?: string) => void; onCancel: () => void }> = ({ onCreate, onCancel }) => {
  ...
  return (
    <div className="border border-white/[0.08] rounded-lg p-3 bg-white/[0.02] space-y-2">
      ...
      <div className="flex gap-1.5">
        <button onClick={() => name.trim() && onCreate(name.trim(), desc.trim() || undefined)}
          className="flex items-center gap-1 px-2 py-1 rounded bg-sage-400/20 text-sage-300 text-[9px] hover:bg-sage-400/30">
          <Check size={10} /> Create
        </button>
        <button onClick={onCancel}
          className="flex items-center gap-1 px-2 py-1 rounded bg-white/[0.06] text-white/50 text-[9px] hover:bg-white/[0.1]">
          <X size={10} /> Cancel
        </button>
      </div>
    </div>
...
const IngestForm: React.FC<{ stackId: string; onDone: () => void }> = ({ stackId, onDone }) => {
    ...
    <div className="border border-white/[0.08] rounded-lg p-3 bg-white/[0.02] space-y-2">
      <div className="flex gap-2">
        <button onClick={() => setMode('text')}
          className={`px-2 py-1 rounded text-[9px] ${mode === 'text' ? 'bg-terra-300/10 text-terra-300' : 'text-white/40'}`}>
          Paste Text
        </button>
        <button onClick={() => setMode('file')}
          className={`px-2 py-1 rounded text-[9px] ${mode === 'file' ? 'bg-terra-300/10 text-terra-300' : 'text-white/40'}`}>
          File Path
        </button>
      </div>
      ...
      {mode === 'file' && (
        <input value={filePath} onChange={e => setFilePath(e.target.value)} placeholder="/path/to/file..." className={`${inputCls} font-mono`} />
      )}

      <button onClick={handleIngest} disabled={ingesting}
        className="flex items-center gap-1 px-2 py-1 rounded bg-terra-300/10 text-terra-300 text-[10px] hover:bg-terra-300/20 disabled:opacity-40">
...
const KnowledgePanel: React.FC = () => {
  ...
  const refreshDocs = useCallback(async () => {
    if (!selectedStackId) { setDocuments([]); return }
  ...
  const handleCreateStack = async (name: string, desc?: string) => {
  ...
  const handleSearch = async () => {
    if (!selectedStackId || !searchQuery.trim()) return
    setSearching(true)
      ...
      <div className="px-4 py-3 border-b border-white/[0.06] space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowCreate(true)}
            className="flex items-center gap-1 px-2 py-1 rounded bg-terra-300/10 text-terra-300 text-[10px] hover:bg-terra-300/20">
        ...
        {stacks.length === 0 && !showCreate && (
          <p className="text-[9px] text-white/25 text-center py-2">No stacks yet. Create one to start building a knowledge base.</p>
        )}
          ...
          <>
            ...
            <div className="flex gap-1.5">
              <input
                ...
                className="flex-1 bg-white/[0.06] border border-white/[0.08] rounded px-2 py-1.5 text-[10px] text-white/70 outline-none"
              />
              <button onClick={handleSearch} disabled={searching}
                className="flex items-center gap-1 px-2 py-1 rounded bg-terra-400/20 text-terra-300 text-[10px] hover:bg-terra-400/30 disabled:opacity-40">
            ...
            <div className="space-y-1">
              ...
              {documents.length === 0 && !showIngest && (
                <p className="text-[9px] text-white/25 text-center py-4">No documents. Ingest files to build the knowledge base.</p>
              )}
