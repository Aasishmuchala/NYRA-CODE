...
import { FileCode, FileText, Table, Image, Database, ExternalLink, RotateCcw, Filter, ArrowUpDown, Loader, } from 'lucide-react';
const typeIcons = {
    code: <FileCode size={20} className="text-blue-400"/>,
    document: <FileText size={20} className="text-white/40"/>,
    spreadsheet: <Table size={20} className="text-emerald-400"/>,
    image: <Image size={20} className="text-purple-400"/>,
    data: <Database size={20} className="text-amber-400"/>,
};
const typeLabels = {
    code: 'Code',
    document: 'Document',
...
export const ArtifactsTab = () => {
            ...
            try {
                ...
                for (const task of tasks) {
                    try {
                        ...
                        allArtifacts.push(...taskArtifacts.map((artifact) => ({
                            id: artifact.id,
    ...
    const uniqueAgents = useMemo(() => Array.from(new Set(artifacts.map((artifact) => artifact.agent))), [artifacts]);
    ...
    const filteredAndSorted = useMemo(() => {
        let filtered = artifacts.filter((artifact) => {
            if (filterType && artifact.type !== filterType)
                return false;
            if (filterAgent && artifact.agent !== filterAgent)
                return false;
            return true;
        });
        // Sort
    ...
    return (<div className="flex flex-col h-full">
      ...
      <div className="border-b border-white/[0.06] bg-white/[0.02] p-4 sticky top-0 z-10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            {loading ? (<Loader size={14} className="text-terra-300 animate-spin"/>) : (<span className="text-xs text-white/30">({filteredAndSorted.length} items)</span>)}
          </div>
        </div>
        ...
        <div className="flex gap-3 flex-wrap items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="px-3 py-1.5 bg-white/[0.05] border border-white/[0.08] rounded-lg text-xs text-white/80 focus:outline-none focus:border-white/15 cursor-pointer">
          ...
          {/* Agent Filter */}
          <select value={filterAgent} onChange={(e) => setFilterAgent(e.target.value)} className="px-3 py-1.5 bg-white/[0.05] border border-white/[0.08] rounded-lg text-xs text-white/80 focus:outline-none focus:border-white/15 cursor-pointer">
          ...
          <div className="flex items-center gap-1 bg-white/[0.05] border border-white/[0.08] rounded-lg p-1">
            <button onClick={() => toggleSort('date')} className={`px-2.5 py-1 rounded text-xs font-medium flex items-center gap-1.5 transition-colors ${sortBy === 'date'
            ? 'bg-white/10 text-white/80'
            : 'text-white/40 hover:text-white/60'}`}>
              Date
              {sortBy === 'date' && (<ArrowUpDown size={12} className={sortOrder === 'asc' ? 'rotate-180' : ''}/>)}
            </button>
            <button onClick={() => toggleSort('name')} className={`px-2.5 py-1 rounded text-xs font-medium flex items-center gap-1.5 transition-colors ${sortBy === 'name'
            ? 'bg-white/10 text-white/80'
            : 'text-white/40 hover:text-white/60'}`}>
              Name
              {sortBy === 'name' && (<ArrowUpDown size={12} className={sortOrder === 'asc' ? 'rotate-180' : ''}/>)}
            </button>
            <button onClick={() => toggleSort('size')} className={`px-2.5 py-1 rounded text-xs font-medium flex items-center gap-1.5 transition-colors ${sortBy === 'size'
            ? 'bg-white/10 text-white/80'
            : 'text-white/40 hover:text-white/60'}`}>
              Size
              {sortBy === 'size' && (<ArrowUpDown size={12} className={sortOrder === 'asc' ? 'rotate-180' : ''}/>)}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (<div className="flex items-center justify-center h-full">
          ...
          </div>) : filteredAndSorted.length === 0 ? (<div className="flex items-center justify-center h-full text-white/30">
            ...
            {filteredAndSorted.map((artifact) => (<div key={artifact.id} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 hover:border-white/[0.12] transition-colors flex flex-col">
                {/* Icon and Type */}
