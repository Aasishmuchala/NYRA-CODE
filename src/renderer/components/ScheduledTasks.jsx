...
export const ScheduledTasks = ({ onClose }) => {
    ...
    useEffect(() => { load(); }, []);
    const add = async () => {
        if (!newPrompt.trim() || !effectiveCron.trim())
            return;
        setSaving(true);
        try {
      ...
      <div className="w-[560px] max-h-[80vh] flex flex-col bg-[#111111] border border-white/10 rounded-2xl shadow-2xl shadow-black/80 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07]">
          <div className="flex items-center gap-2.5">
            ...
            {tasks.length > 0 && (<span className="bg-white/10 text-white/50 text-[10px] px-1.5 py-0.5 rounded-full font-medium">
                {tasks.length}
              </span>)}
          </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          {tasks.length === 0 && !creating && (<div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          {tasks.map(task => (<div key={task.id} className="border-b border-white/[0.06] last:border-0">
              {/* Row */}
              <div className="flex items-center gap-3 px-5 py-3.5 hover:bg-white/[0.03] transition-colors">
                {/* Toggle */}
                <button onClick={() => toggle(task)} className={`flex-shrink-0 transition-colors ${task.enabled ? 'text-terra-400' : 'text-white/20'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-0.5">
                    ...
                    {task.lastRun && (<span className="text-white/20 text-[10px]">last: {formatRelative(task.lastRun)}</span>)}
                  </div>
                </div>

                {/* Actions */}
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => runNow(task)} title="Run now" className="p-1.5 text-white/20 hover:text-green-400 transition-colors">
                    <Play size={13}/>
                  </button>
                  <button onClick={() => setExpandedId(expandedId === task.id ? null : task.id)} className="p-1.5 text-white/20 hover:text-white/50 transition-colors">
                    {expandedId === task.id ? <ChevronUp size={13}/> : <ChevronDown size={13}/>}
                  </button>
                  <button onClick={() => remove(task.id)} className="p-1.5 text-white/20 hover:text-red-400 transition-colors">
                    <Trash2 size={13}/>
                  </button>
                </div>
              </div>
              ...
              {expandedId === task.id && (<div className="px-5 pb-3">
        ...
        {/* Create form */}
        {creating && (<div className="border-t border-white/[0.07] px-5 py-4 space-y-3 bg-white/[0.02]">
            <p className="text-white/60 text-xs font-semibold uppercase tracking-wider">New Task</p>

            {/* Label */}
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Task name (optional)" className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-white/80 placeholder-white/20 outline-none focus:border-terra-400/50"/>

            {/* Prompt */}
            <textarea value={newPrompt} onChange={e => setNewPrompt(e.target.value)} placeholder="Prompt to run automatically…" rows={3} className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-white/80 placeholder-white/20 outline-none focus:border-terra-400/50 resize-none"/>

            {/* Schedule */}
            <div className="space-y-2">
              <label className="text-white/40 text-[11px]">Schedule</label>
              <div className="grid grid-cols-2 gap-1.5">
                {CRON_PRESETS.map(p => (<button key={p.cron} onClick={() => setSelectedPreset(p.cron)} className={`text-left px-3 py-2 rounded-lg text-[11px] border transition-all ${selectedPreset === p.cron
                    ? 'border-terra-400/50 bg-terra-400/10 text-terra-300'
                    : 'border-white/[0.07] bg-white/[0.03] text-white/40 hover:text-white/60'}`}>
                    {p.label}
                  </button>))}
              </div>
              {selectedPreset === 'custom' && (<input value={customCron} onChange={e => setCustomCron(e.target.value)} placeholder="cron expression  e.g. 0 9 * * 1-5" className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-white/80 placeholder-white/20 font-mono outline-none focus:border-terra-400/50"/>)}
            </div>

            {/* Buttons */}
            <div className="flex gap-2 pt-1">
              <button onClick={() => { setCreating(false); setNewPrompt(''); setNewName(''); }} className="flex-1 py-2 rounded-lg border border-white/10 text-white/40 text-xs hover:text-white/60 transition-colors">
                Cancel
              </button>
              <button onClick={add} disabled={!newPrompt.trim() || saving} className="flex-1 py-2 rounded-lg bg-terra-500 hover:bg-terra-400 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-medium transition-colors">
        ...
        {/* Footer */}
        {!creating && (<div className="border-t border-white/[0.07] px-5 py-3">
            <button onClick={() => setCreating(true)} className="flex items-center gap-2 text-terra-400 hover:text-terra-300 text-xs font-medium transition-colors">
              <Plus size={14}/>
              New scheduled task
            </button>
          </div>)}
