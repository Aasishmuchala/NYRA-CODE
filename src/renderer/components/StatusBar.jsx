...
export const StatusBar = ({ status, wsStatus, wsUrl, log }) => {
    const statusColor = {
        idle: 'text-white/30',
    ...
    const wsIcon = wsConnected ? <CheckCircle2 size={11} className="text-green-400"/> :
        status === 'error' ? <AlertCircle size={11} className="text-red-400"/> :
            status === 'idle' ? <WifiOff size={11} className="text-white/25"/> :
                wsStatus === 'error' ? <AlertCircle size={11} className="text-red-400"/> :
                    <Loader2 size={11} className="text-yellow-400 animate-spin"/>;
    const wsLabel = wsUrl ? wsUrl.replace('ws://', '') : '—';
    return (<div className="h-6 flex items-center px-3 gap-3 border-t border-white/[0.05] bg-black/20 flex-shrink-0">
