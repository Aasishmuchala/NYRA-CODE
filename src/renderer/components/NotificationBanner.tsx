...
export const NotificationBanner: React.FC = () => {
  ...
  const dismiss = (id: string) => setBanners(b => b.filter(x => x.id !== id))

  const push = (b: Omit<Banner, 'id'>) => {
    const id = `banner-${Date.now()}`
    setBanners(prev => [...prev, { ...b, id }])
    // Auto-dismiss info after 5s
    if (b.type === 'info') setTimeout(() => dismiss(id), 5000)
  }
    ...
    <div className="fixed top-10 right-4 z-50 flex flex-col gap-2 max-w-xs">
      {banners.map(b => {
        ...
        return (
          <div
            key={b.id}
            className={`flex items-start gap-3 bg-[#1c1c1c] border ${colors[b.type]} rounded-xl px-4 py-3 shadow-2xl shadow-black/60 animate-in slide-in-from-top-2`}
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {b.action && (
                <button
                  ...
                  className="mt-2 text-xs text-terra-400 hover:text-terra-300 font-medium transition-colors"
                >
                  {b.action.label}
                </button>
              )}
