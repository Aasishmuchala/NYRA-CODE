...
import { Search, Plus, Settings, Clock, BookOpen, Download, Hash, ArrowRight, Mic, FileText } from 'lucide-react';
export const CommandPalette = ({ onClose, sessions, onSelectSession, onNewChat, onOpenSettings, onOpenScheduled, onOpenPrompts, onOpenExport, onStartVoice, searchSessions, }) => {
    ...
    const matchedSessions = q
        ...
        : sessions.slice(0, 5).map(s => ({ session: s, score: 1, matchedMsg: undefined }));
    const matchedPrompts = q
        ? prompts.filter(p => p.title.toLowerCase().includes(q) || p.content.toLowerCase().includes(q)).slice(0, 4)
        : prompts.slice(0, 3);
    const allItems = [
        ...matchedCommands.map(c => ({ kind: 'cmd', data: c })),
        ...matchedSessions.map(r => ({ kind: 'session', data: r })),
        ...matchedPrompts.map(p => ({ kind: 'prompt', data: p })),
    ];
    const clampedCursor = Math.min(cursor, allItems.length - 1);
    // Scroll active item into view
    useEffect(() => {
        const el = listRef.current?.querySelector(`[data-idx="${clampedCursor}"]`);
    ...
    const handleKey = useCallback((e) => {
        ...
        if (e.key === 'Enter' && allItems[clampedCursor]) {
            executeItem(allItems[clampedCursor]);
        }
        if (e.key === 'Escape')
            onClose();
    }, [allItems, clampedCursor, executeItem, onClose]);
    return (<div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-[580px] max-h-[480px] bg-[#141414] border border-white/10 rounded-2xl shadow-2xl shadow-black/80 overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        {/* Search input */}
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/[0.07]">
          <Search size={15} className="text-white/30 flex-shrink-0"/>
          <input ref={inputRef} value={query} onChange={e => { setQuery(e.target.value); setCursor(0); }} onKeyDown={handleKey} placeholder="Search sessions, commands, prompts…" className="flex-1 bg-transparent text-sm text-white/90 placeholder-white/25 outline-none"/>
        ...
        <div ref={listRef} className="overflow-y-auto flex-1 py-1.5 scrollbar-thin">
          ...
          {/* Commands */}
          {matchedCommands.length > 0 && (<Section label="Commands">
              {matchedCommands.map((cmd, i) => {
                const idx = i;
                return (<ResultRow key={cmd.id} idx={idx} active={idx === clampedCursor} icon={cmd.icon} label={cmd.label} meta={cmd.shortcut} onClick={() => executeItem({ kind: 'cmd', data: cmd })}/>);
            })}
            </Section>)}

          {/* Sessions */}
          {matchedSessions.length > 0 && (<Section label="Chats">
              {matchedSessions.map((r, i) => {
                ...
                return (<ResultRow key={r.session.id} idx={idx} active={idx === clampedCursor} icon={<Hash size={13}/>} label={r.session.title || 'New chat'} meta={preview || undefined} onClick={() => executeItem({ kind: 'session', data: r })}/>);
            })}
            </Section>)}

          {/* Prompts */}
          {matchedPrompts.length > 0 && (<Section label="Saved Prompts">
              {matchedPrompts.map((p, i) => {
                const idx = matchedCommands.length + matchedSessions.length + i;
                return (<ResultRow key={p.id} idx={idx} active={idx === clampedCursor} icon={<FileText size={13}/>} label={p.title} meta="Click to copy" onClick={() => executeItem({ kind: 'prompt', data: p })}/>);
            })}
            </Section>)}

          {allItems.length === 0 && (<p className="text-white/25 text-xs text-center py-8">No results for "{query}"</p>)}
        </div>
...
// ── Helpers ───────────────────────────────────────────────────────────────────
const Section = ({ label, children }) => (<div className="mb-1">
    <p className="px-4 py-1.5 text-[9px] font-semibold text-white/25 uppercase tracking-widest">{label}</p>
    {children}
  </div>);
const ResultRow = ({ idx, active, icon, label, meta, onClick }) => (<button data-idx={idx} onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-2 text-left transition-colors ${active ? 'bg-terra-400/15 text-white' : 'text-white/60 hover:bg-white/[0.04] hover:text-white/90'}`}>
