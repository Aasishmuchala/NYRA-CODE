...
export const SlashCommandPicker = ({ query, onSelect, onClose, visible }) => {
    ...
    const filtered = useMemo(() => {
        if (!query)
            return allItems.slice(0, 12); // show top 12 by default
        const q = query.toLowerCase();
        return allItems.filter(s => s.name.toLowerCase().includes(q) ||
            s.description.toLowerCase().includes(q) ||
            s.category.toLowerCase().includes(q)).slice(0, 12);
    }, [allItems, query]);
    ...
    if (!visible || filtered.length === 0)
        return null;
    return (<div className="absolute bottom-full left-0 right-0 mb-2 z-50">
      <div className="max-w-[720px] mx-auto">
        <div className="bg-[#141210] border border-white/[0.08] rounded-xl shadow-2xl shadow-black/60 overflow-hidden">
          ...
          <div ref={listRef} className="max-h-[280px] overflow-y-auto scrollbar-thin py-1">
            {filtered.map((skill, i) => (<button key={skill.id} onClick={() => onSelect(skill)} onMouseEnter={() => setSelectedIndex(i)} className={`w-full flex items-center gap-3 px-3 py-2 text-left transition-colors ${i === selectedIndex ? 'bg-white/[0.06]' : 'hover:bg-white/[0.03]'}`}>
                {/* Icon */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {skill.category !== 'builtin' && (<span className="text-[9px] text-white/15 capitalize">{skill.category}</span>)}
                  </div>
