...
export const SettingsPanel = ({ onClose }) => {
    ...
    const [theme, setThemeState] = useState({ mode: 'dark', accent: 'indigo', fontSize: 'md', wallpaper: 'herringbone' });
    // ── Provider state ────────────────────────────────────────────────────────
    ...
    const [gwPing, setGwPing] = useState(null);
    ...
    const [guardScanning, setGuardScanning] = useState(null); // 'security' | 'stability' | 'threat' | 'full'
    const [guardResults, setGuardResults] = useState([]);
    const [guardLogs, setGuardLogs] = useState([]);
    const [guardStatus, setGuardStatus] = useState({ enabled: false, scanning: false, lastScan: null, issueCount: 0 });
    const [guardDiagnosing, setGuardDiagnosing] = useState(false);
    const [guardDiagnosis, setGuardDiagnosis] = useState(null);
    useEffect(() => {
        ...
        const cleanups = [];
        window.nyra.openclaw.getStatus().then((s) => setGwStatus(s));
        cleanups.push(window.nyra.openclaw.onStatusChange((s) => {
            setGwStatus(s);
            if (s === 'running')
                setGwRestarting(false);
        }));
        cleanups.push(window.nyra.openclaw.onLog?.((line) => {
            setGwLogs(prev => [...prev.slice(-80), line]);
        }));
        cleanups.push(window.nyra.openclaw.onError?.((msg) => {
        ...
        cleanups.push(window.nyra.openclaw.onRestarting?.((info) => {
        ...
        // ── NyraGuard init ──────────────────────────────────────────────────
        window.nyra.guard?.getConfig?.().then((cfg) => {
            if (cfg) {
        ...
        window.nyra.guard?.status?.().then((s) => { if (s?.hasApiKey)
            setGuardHasKey(s.hasApiKey); if (s)
            setGuardStatus(s); }).catch(() => { });
        window.nyra.guard?.getLog?.().then((logs) => { if (logs)
            setGuardLogs(logs.slice(-50)); }).catch(() => { });
        // Guard event listeners
        const guardCleanups = [];
        if (window.nyra.guard?.onScanComplete) {
            guardCleanups.push(window.nyra.guard.onScanComplete((result) => {
        ...
        if (window.nyra.guard?.onIssueDetected) {
            guardCleanups.push(window.nyra.guard.onIssueDetected((issue) => {
                setGuardResults(prev => [...prev, issue]);
            }));
        }
        if (window.nyra.guard?.onLog) {
            guardCleanups.push(window.nyra.guard.onLog((log) => {
    ...
    const guardSaveApiKey = useCallback(async () => {
        ...
        try {
            ...
            console.error('Guard API key save failed:', e);
        }
        setGuardSavingKey(false);
    }, [guardApiKey]);
    const guardRunScan = useCallback(async (type) => {
        setGuardScanning(type);
        setGuardResults([]);
        try {
            let result = [];
            if (type === 'security')
                result = await window.nyra.guard?.scanSecurity?.();
            else if (type === 'stability')
                result = await window.nyra.guard?.scanStability?.();
            else if (type === 'threat')
                result = await window.nyra.guard?.scanThreat?.();
            else if (type === 'full')
                result = await window.nyra.guard?.scanAll?.();
            if (result) {
    ...
    const guardToggleEnabled = useCallback(async (enabled) => {
        setGuardEnabled(enabled);
        await window.nyra.guard?.setConfig?.({ enabled });
    }, []);
    const guardToggleAutoScan = useCallback(async (enabled) => {
    ...
    useEffect(() => { if (tab === 'gateway') { runGwPing(); } }, [tab, runGwPing]);
    const addServer = async () => {
        if (!newName.trim() || !newCmd.trim())
            return;
        const server = { command: newCmd.trim(), args: newArgs.trim() ? newArgs.split(' ') : [] };
    ...
    return (<div className="flex flex-col h-full bg-black/50 backdrop-blur-md border-l border-white/[0.06]">
      {/* Header */}
      ...
      <div className="flex gap-1 px-5 pt-3 flex-wrap">
        {['providers', 'gateway', 'mcp', 'ollama', 'plugins', 'skills', 'appearance', 'guard', 'about'].map(t => {
            const labels = {
                providers: 'Providers', gateway: 'Gateway', mcp: 'MCP Servers', ollama: 'Local LLMs',
                plugins: 'Plugins', skills: 'Skills', appearance: 'Appearance', guard: 'NyraGuard', about: 'About',
            };
            return (<button key={t} onClick={() => setTab(t)} className={`px-3 py-1.5 text-xs rounded-lg font-medium transition-colors ${tab === t ? 'bg-white/10 text-white' : 'text-white/35 hover:text-white/70'}`}>
              {labels[t]}
            </button>);
        })}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 scrollbar-thin">
        ...
        {/* ── Providers Tab ─────────────────────────────────────────────────── */}
        {tab === 'providers' && (<div className="space-y-4">
            ...
            {providerCatalog.map(def => {
                ...
                return (<div key={def.id} className={`border rounded-xl overflow-hidden transition-all ${isConnected
                        ? 'border-sage-500/25 bg-sage-500/[0.03]'
                        : 'border-white/[0.07] bg-white/[0.02]'}`}>
                  {/* Provider header */}
                  <button onClick={() => setExpandedProvider(isExpanded ? null : def.id)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-white/[0.03] transition-colors">
                    ...
                    {isConnected && (<span className="w-2 h-2 rounded-full bg-sage-400 flex-shrink-0"/>)}
                    <ChevronDown size={13} className={`text-white/25 transition-transform ${isExpanded ? 'rotate-180' : ''}`}/>
                  </button>

                  {/* Expanded section */}
                  {isExpanded && (<div className="px-4 pb-4 pt-1 space-y-3 border-t border-white/[0.05]">
                      {/* OAuth sign-in button — shown for providers with OAuth support */}
                      {oauthAvail[def.id] && !isConnected && (<div className="space-y-2">
                          <button onClick={async () => {
                            ...
                            }} disabled={oauthLoading === def.id} className={`w-full flex items-center justify-center gap-2 px-3 py-2.5 text-white text-xs rounded-xl transition-colors font-medium ${oauthLoading === def.id ? 'bg-warm-800/40' :
                            ...
                            {oauthLoading === def.id
                                ...
                                : <><ExternalLink size={12}/> {def.id === 'openai' ? 'Sign in with ChatGPT' :
                                        def.id === 'gemini' ? 'Sign in with Google' :
                                            def.id === 'copilot' ? 'Sign in with GitHub' :
                                                `Sign in with ${def.label}`}</>}
                          </button>
                          {/* GitHub device code display */}
                          {deviceCode && oauthLoading === 'copilot' && (<div className="bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-center space-y-1">
                          ...
                          {/* OAuth error display */}
                          {providerErrors[def.id] && (<div className="bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2 text-[11px] text-red-400">
                      ...
                      {!oauthAvail[def.id] && def.oauthUrl && !isConnected && (<button onClick={() => window.nyra.providers.openOauth(def.oauthUrl)} className="w-full flex items-center justify-center gap-2 px-3 py-2.5 text-white text-xs rounded-xl transition-colors font-medium bg-terra-600/80 hover:bg-terra-500">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex gap-2">
                          <input type="password" value={keyInputs[def.id] ?? ''} onChange={e => setKeyInputs(prev => ({ ...prev, [def.id]: e.target.value }))} placeholder={def.apiKeyPrefix ? `${def.apiKeyPrefix}...` : 'Paste your API key'} className="flex-1 bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-white/80 placeholder-white/20 outline-none focus:border-terra-400/50 font-mono"/>
                          <button onClick={async () => {
                        ...
                        {isConnected && (<p className="text-[10px] text-sage-400/60 flex items-center gap-1">
                            <Check size={9}/> Key saved (encrypted via macOS Keychain)
                          </p>)}
                      </div>

                      {/* Model selector */}
                      {def.models.length > 1 && (<div className="space-y-1.5">
                          <label className="text-[10px] text-white/35 uppercase tracking-widest">Model</label>
                          <div className="grid grid-cols-2 gap-1.5">
                            {def.models.map(m => (<button key={m.id} onClick={async () => {
                                    await window.nyra.providers.setModel(def.id, m.id);
                                    setProviderStates(await window.nyra.providers.list());
                                }} className={`px-2.5 py-1.5 rounded-lg text-[11px] border transition-all text-left ${(state?.activeModel ?? def.models[0]?.id) === m.id
                                    ? 'border-terra-400/50 bg-terra-400/10 text-white'
                                    : 'border-white/[0.06] text-white/40 hover:border-white/15 hover:text-white/60'}`}>
                                {m.label}
                                {m.contextWindow && (<span className="text-[9px] text-white/20 ml-1">{Math.round(m.contextWindow / 1000)}K</span>)}
                              </button>))}
                          </div>
                        </div>)}
                      ...
                      {isConnected && (
                        <div className="flex items-center gap-4 pt-1 border-t border-white/[0.05] mt-1">
                          {oauthAvail[def.id] && (
                            <button onClick={async () => {
                                setOauthLoading(def.id);
                                setProviderErrors(prev => { const n = { ...prev }; delete n[def.id]; return n; });
                            ...
                            }} disabled={oauthLoading === def.id} className="flex items-center gap-1.5 text-terra-300/70 hover:text-terra-300 text-[11px] transition-colors cursor-pointer disabled:opacity-40">
                              {oauthLoading === def.id
                                ? <><Loader2 size={10} className="animate-spin"/> Switching...</>
                                : <><RotateCw size={10}/> Switch account</>}
                            </button>
                          )}
                          <button onClick={async () => {
                                await window.nyra.providers.removeKey(def.id);
                                setProviderStates(await window.nyra.providers.list());
                            }} className="flex items-center gap-1.5 text-blush-400/70 hover:text-blush-400 text-[11px] transition-colors cursor-pointer ml-auto">
        ...
        {/* ── Gateway Tab ──────────────────────────────────────────────────── */}
        {tab === 'gateway' && (<div className="space-y-4">
            ...
            {(() => {
              ...
              const modelOk = mt?.tested ? mt.ok : true;
              ...
              return (
                <div className={`border rounded-xl px-4 py-3.5 transition-all ${
                  canConverse ? 'border-sage-500/30 bg-sage-500/[0.06]'
                    : gwPinging ? 'border-gold-400/20 bg-gold-400/[0.04]'
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-white font-semibold">
                        {gwPinging ? 'Testing connection...' : canConverse ? 'Ready to Converse' : 'Not Ready'}
                      </p>
                      <p className="text-[10px] text-white/40 mt-0.5">
                        {gwPinging ? 'Checking gateway, proxy, providers, and model...' :
                         canConverse ? `Gateway connected · ${readyProviders.length} provider${readyProviders.length > 1 ? 's' : ''} · model verified`
                    ...
                    <button onClick={runGwPing} disabled={gwPinging}
                    ...
                    <div className="mt-3 pt-3 border-t border-white/[0.06] space-y-2">
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[10px]">
                        <div className="flex items-center gap-1.5">
                          {gwRunning ? <Check size={10} className="text-sage-400" /> : <AlertTriangle size={10} className="text-blush-400" />}
                          <span className={gwRunning ? 'text-white/60' : 'text-blush-300/70'}>Gateway Process</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          {proxyOk ? <Check size={10} className="text-sage-400" /> : <AlertTriangle size={10} className="text-blush-400" />}
                          <span className={proxyOk ? 'text-white/60' : 'text-blush-300/70'}>WebSocket Proxy</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          {readyProviders.length > 0 ? <Check size={10} className="text-sage-400" /> : <AlertTriangle size={10} className="text-blush-400" />}
                      ...
                      {mt?.tested && !mt.ok && mt.error && (
                        <div className="bg-blush-400/10 border border-blush-400/20 rounded-lg px-3 py-2 text-[10px] text-blush-300 leading-relaxed">
                          <span className="font-semibold">Model error:</span>{' '}{mt.error}
                          {mt.model && <span className="block mt-1 text-white/30 font-mono">{mt.model}</span>}
                        </div>
                      )}
                    </div>
                  )}
            ...
            <div className="flex items-center gap-3 border border-white/[0.07] bg-white/[0.02] rounded-xl px-4 py-3">
              <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${gwStatus === 'running' ? 'bg-sage-400 animate-pulse' :
            ...
            <div className="flex gap-2">
              <button onClick={async () => {
                ...
                try { await window.nyra.openclaw.restart(); }
                catch { setGwRestarting(false); }
            }} disabled={gwRestarting} className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 text-white text-xs rounded-xl transition-colors font-medium bg-terra-600/80 hover:bg-terra-500 disabled:bg-white/[0.05] disabled:text-white/20">
                {gwRestarting
            ...
            {/* Log viewer */}
            {gwLogs.length > 0 && (<div className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-white/50 text-[10px] font-semibold uppercase tracking-widest">Gateway Log</p>
                  <button onClick={() => setGwLogs([])} className="text-[10px] text-white/25 hover:text-white/50 transition-colors">
                    Clear
                  </button>
                </div>
                <div className="bg-black/40 border border-white/[0.06] rounded-xl p-3 max-h-48 overflow-y-auto scrollbar-thin font-mono text-[10px] leading-relaxed text-white/40 space-y-0.5">
                  {gwLogs.map((line, i) => (<div key={i} className={line.startsWith('[ERROR]') ? 'text-blush-300/70' :
        ...
        {tab === 'mcp' && (<div className="space-y-4">
            ...
            <div className="space-y-2">
              {Object.entries(servers).map(([name, cfg]) => (<div key={name} className="flex items-center justify-between bg-white/[0.04] border border-white/[0.07] rounded-xl px-3 py-2.5">
                  <div>
              ...
              {Object.keys(servers).length === 0 && (<p className="text-white/20 text-xs text-center py-4">No MCP servers configured</p>)}
            </div>
            ...
            <div className="border border-white/[0.08] rounded-xl p-4 space-y-3">
              ...
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Name (e.g. filesystem)" className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-3 py-2 text-sm text-white/80 placeholder-white/20 outline-none focus:border-terra-400/50"/>
              <input value={newCmd} onChange={e => setNewCmd(e.target.value)} placeholder="Command (e.g. npx)" className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-3 py-2 text-sm text-white/80 placeholder-white/20 outline-none focus:border-terra-400/50 font-mono"/>
              <input value={newArgs} onChange={e => setNewArgs(e.target.value)} placeholder="Args (space-separated)" className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-3 py-2 text-sm text-white/80 placeholder-white/20 outline-none focus:border-terra-400/50 font-mono"/>
        ...
        {/* ── Ollama Tab ──────────────────────────────────────────────────── */}
        {tab === 'ollama' && (<div className="space-y-4">
            {/* Status indicator */}
            <div className="flex items-center gap-3 border border-white/[0.07] bg-white/[0.02] rounded-xl px-4 py-3">
              ...
              <div className="flex-1">
                ...
                {!ollamaOnline && (<p className="text-xs text-white/30 mt-0.5">Install Ollama from ollama.com to run models locally</p>)}
              </div>
              {!ollamaOnline && (<button onClick={() => window.nyra.app.openExternal('https://ollama.com')} className="px-3 py-2 bg-terra-400 hover:bg-terra-500 text-white text-xs rounded-lg transition-colors font-medium flex items-center gap-1.5">
            ...
            {ollamaOnline && (<>
                ...
                {ollamaModels.length > 0 && (<div className="space-y-2">
                    <p className="text-white/50 text-[10px] font-semibold uppercase tracking-widest">Installed Models</p>
                    <div className="space-y-2">
                      {ollamaModels.map(model => (<div key={model.id} className="flex items-center justify-between border border-white/[0.07] bg-white/[0.02] rounded-lg px-3 py-2.5">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex gap-3 mt-0.5 text-[10px] text-white/30">
                              {model.size && (<span className="flex items-center gap-1">
                                  <HardDrive size={9}/> {(model.size / 1e9).toFixed(1)} GB
                                </span>)}
                              {model.parameterSize && (<span className="flex items-center gap-1">
                    ...
                    }} disabled={syncing} className={`w-full flex items-center justify-center gap-2 px-3 py-2 text-white text-xs rounded-lg transition-colors font-medium mt-2 ${syncing ? 'bg-warm-800/40' : 'bg-terra-400 hover:bg-terra-500'}`}>
                      {syncing
                        ? <><Loader2 size={12} className="animate-spin"/> Syncing...</>
                        : <><Download size={12}/> Sync to OpenClaw</>}
                    </button>
                    {syncResult && (<div className={`mt-2 px-3 py-2 rounded-lg text-[11px] ${syncResult.success
                ...
                <div className="border border-white/[0.08] rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input type="text" value={pullModelName} onChange={e => setPullModelName(e.target.value)} onKeyPress={e => {
                ...
                }} placeholder="e.g. llama2, mistral:7b" className="flex-1 bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-white/80 placeholder-white/20 outline-none focus:border-terra-400/50"/>
                    <button onClick={() => {
                ...
                }} disabled={!pullModelName.trim() || !!pullingModel} className="px-3 py-2 bg-terra-400 hover:bg-terra-500 disabled:bg-white/[0.05] disabled:text-white/20 text-white text-xs rounded-lg transition-colors font-medium flex items-center gap-1.5">
                  ...
                  {/* Pull progress */}
                  {pullingModel && pullProgress && (<div className="space-y-2">
                      <div className="space-y-1">
                        <p className="text-xs text-white/60">{pullProgress.status}</p>
                        {pullProgress.total && pullProgress.completed !== undefined && (<div className="w-full bg-white/[0.05] rounded-full h-1.5 overflow-hidden">
                            <div className="h-full bg-terra-500 transition-all" style={{ width: `${(pullProgress.completed / pullProgress.total) * 100}%` }}/>
                          </div>)}
                      </div>
                    </div>)}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['llama3.1:8b', 'mistral:7b', 'codellama:13b', 'deepseek-coder:6.7b', 'phi3:mini', 'gemma2:9b'].map(modelName => (<button key={modelName} onClick={() => {
                        setPullingModel(modelName);
                        window.nyra.ollama.pull(modelName);
                    }} disabled={pullingModel === modelName || !!pullingModel} className="px-3 py-2 border border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.04] disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs rounded-lg transition-colors text-left truncate">
                        {modelName}
                      </button>))}
                  </div>
                </div>
        ...
        {/* ── Appearance Tab ───────────────────────────────────────────────── */}
        {tab === 'appearance' && (<div className="space-y-6">
            {/* Mode */}
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                ...
                { id: 'dim', label: 'Dim', icon: Eye },
                { id: 'light', label: 'Light', icon: Sun },
                { id: 'auto', label: 'Auto', icon: Monitor },
            ]).map(({ id, label, icon: Icon }) => (<button key={id} onClick={() => applyTheme({ mode: id })} className={`flex flex-col items-center gap-1 px-3 py-2.5 rounded-xl text-xs border transition-all ${theme.mode === id
                    ? 'border-terra-400/50 bg-terra-400/10 text-white'
              ...
              {theme.mode === 'auto' && (<p className="text-[10px] text-white/25 mt-1.5 leading-relaxed">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {ACCENT_COLORS.map(c => (<button key={c.id} onClick={() => applyTheme({ accent: c.id })} title={c.label} className={`w-7 h-7 rounded-full ${c.tw} transition-all flex items-center justify-center ${theme.accent === c.id
            ...
            <div>
              ...
              <div className="flex gap-2">
                {['sm', 'md', 'lg'].map(s => (<button key={s} onClick={() => applyTheme({ fontSize: s })} className={`px-4 py-2 rounded-xl text-xs border transition-all ${theme.fontSize === s
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {WALLPAPER_OPTIONS.map(wp => (<button key={wp.id} onClick={() => applyTheme({ wallpaper: wp.id })} title={wp.label} className={`relative rounded-xl overflow-hidden border-2 transition-all aspect-square ${theme.wallpaper === wp.id
                    ? 'border-terra-400 ring-1 ring-terra-400/30 scale-[1.02]'
                    : 'border-white/[0.08] hover:border-white/20 hover:scale-[1.01]'}`}>
                    {wp.thumb ? (<img src={wp.thumb} alt={wp.label} className="w-full h-full object-cover" loading="lazy"/>) : (<div className="w-full h-full bg-[#131110] flex items-center justify-center">
                        <span className="text-white/20 text-[10px]">None</span>
                      </div>)}
                    {theme.wallpaper === wp.id && (<div className="absolute inset-0 flex items-center justify-center bg-black/30">
        ...
        {/* ── NyraGuard Tab ──────────────────────────────────────────────── */}
        {tab === 'guard' && (<div className="space-y-5">
            {/* Status header */}
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => guardToggleEnabled(!guardEnabled)} className={`relative w-10 h-5 rounded-full transition-colors ${guardEnabled ? 'bg-sage-500' : 'bg-white/10'}`}>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="password" value={guardApiKey} onChange={e => setGuardApiKey(e.target.value)} placeholder={guardHasKey ? '••••••••••• (key saved)' : 'sk-or-v1-...'} className="flex-1 bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 text-xs text-white/80 placeholder-white/20 outline-none focus:border-terra-400/40" onKeyDown={e => e.key === 'Enter' && guardSaveApiKey()}/>
                <button onClick={guardSaveApiKey} disabled={!guardApiKey.trim() || guardSavingKey} className="px-3 py-2 bg-terra-400/15 hover:bg-terra-400/25 border border-terra-400/25 rounded-xl text-xs text-terra-300 font-medium transition-colors disabled:opacity-30">
            ...
            <div>
              ...
              <select value={guardModel} onChange={e => {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {([
                { type: 'security', icon: <Shield size={12}/>, label: 'Security', color: 'terra' },
                { type: 'stability', icon: <Activity size={12}/>, label: 'Stability', color: 'sage' },
                { type: 'threat', icon: <AlertTriangle size={12}/>, label: 'Threat', color: 'gold' },
                { type: 'full', icon: <Zap size={12}/>, label: 'Full Scan', color: 'blush' },
            ]).map(s => (<button key={s.type} onClick={() => guardRunScan(s.type)} disabled={!!guardScanning} className={`flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border text-xs font-medium transition-all ${guardScanning === s.type
            ...
            {guardResults.length > 0 && (<div>
                ...
                <div className="space-y-1.5 max-h-[200px] overflow-y-auto scrollbar-thin">
                  {guardResults.map((r, i) => (<div key={i} className={`flex items-start gap-2 px-3 py-2 rounded-xl border text-xs ${r.severity === 'critical' ? 'bg-red-500/10 border-red-500/20 text-red-300' :
            ...
            <div className="flex items-center justify-between py-3 border-t border-white/[0.06]">
              ...
              <button onClick={() => guardToggleAutoScan(!guardAutoScan)} className={`relative w-10 h-5 rounded-full transition-colors ${guardAutoScan ? 'bg-terra-500' : 'bg-white/10'}`}>
            ...
            {guardAutoScan && (<div className="flex items-center gap-2">
                <p className="text-[10px] text-white/35">Interval (min):</p>
                {[15, 30, 60].map(m => (<button key={m} onClick={() => {
                        setGuardInterval(m);
                        window.nyra.guard?.setConfig?.({ scanInterval: m });
                        if (guardAutoScan)
                            window.nyra.guard?.startAuto?.();
                    }} className={`px-3 py-1 rounded-lg text-[10px] border transition-all ${guardInterval === m
            ...
            <div className="border-t border-white/[0.06] pt-4">
              ...
              <button onClick={guardRunDiagnosis} disabled={guardDiagnosing || !guardHasKey} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-terra-500/15 to-gold-500/15 hover:from-terra-500/25 hover:to-gold-500/25 border border-terra-400/20 rounded-xl text-xs text-white/70 font-medium transition-all disabled:opacity-30">
                {guardDiagnosing ? <Loader2 size={12} className="animate-spin"/> : <Sparkles size={12}/>}
                {guardDiagnosing ? 'Analyzing…' : 'Run AI Diagnosis'}
              </button>
              {!guardHasKey && (<p className="text-[9px] text-blush-400/60 mt-1.5">Set your OpenRouter API key above to enable AI diagnostics.</p>)}
              {guardDiagnosis && (<div className="mt-3 bg-white/[0.03] border border-white/[0.06] rounded-xl p-3 max-h-[180px] overflow-y-auto scrollbar-thin">
                  <pre className="text-[10px] text-white/60 whitespace-pre-wrap font-mono leading-relaxed">{guardDiagnosis}</pre>
                </div>)}
            </div>

            {/* Recent logs */}
            {guardLogs.length > 0 && (<div className="border-t border-white/[0.06] pt-4">
                ...
                <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-2 max-h-[120px] overflow-y-auto scrollbar-thin space-y-0.5">
                  {guardLogs.slice(-20).reverse().map((log, i) => (<div key={i} className="flex items-start gap-2 text-[9px]">
                      ...
                      <span className={`flex-shrink-0 uppercase font-bold ${log.level === 'error' ? 'text-red-400/60' :
                        log.level === 'warn' ? 'text-gold-400/60' :
                            'text-white/20'}`}>{log.level}</span>
        ...
        {tab === 'about' && (<div className="space-y-5">
            ...
            <div className="space-y-2">
              <button onClick={() => window.nyra.app.openExternal('https://openclaw.ai')} className="flex items-center gap-2 text-terra-300 hover:text-terra-300 text-xs transition-colors">
                <ExternalLink size={11}/> OpenClaw Documentation
              </button>
              <button onClick={() => window.nyra.app.openExternal('https://github.com')} className="flex items-center gap-2 text-white/30 hover:text-white/60 text-xs transition-colors">
