...
const OpenAILogo: React.FC<{ size?: number; className?: string }> = ({ size = 14, className }) => (
...
const CopilotLogo: React.FC<{ size?: number; className?: string }> = ({ size = 14, className }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
...
const GeminiLogo: React.FC<{ size?: number; className?: string }> = ({ size = 14, className }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
...
const AnthropicLogo: React.FC<{ size?: number; className?: string }> = ({ size = 14, className }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
...
const OpenRouterLogo: React.FC<{ size?: number; className?: string }> = ({ size = 14, className }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
    <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)

const OllamaLogo: React.FC<{ size?: number; className?: string }> = ({ size = 14, className }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
...
const PROVIDER_LOGOS: Record<string, React.FC<{ size?: number; className?: string }>> = {
  OpenAI: OpenAILogo,
  Copilot: CopilotLogo,
  Gemini: GeminiLogo,
...
export interface ModelDef {
  ...
  name: string
  provider: string
  providerIcon: string       // emoji fallback
  description: string
  costMultiplier: number     // relative cost: 1x = baseline, 3x = 3× baseline, etc.
  tier: 'fast' | 'standard' | 'premium' | 'flagship'
  available: boolean
...
const OPENAI_MODELS: Omit<ModelDef, 'available'>[] = [
  // ── Latest (March 2026) ──
...
const COPILOT_MODELS: Omit<ModelDef, 'available'>[] = [
  // ── Latest (March 2026) ──
...
const GEMINI_MODELS: Omit<ModelDef, 'available'>[] = [
  // ── Latest (March 2026) ──
...
const ANTHROPIC_MODELS: Omit<ModelDef, 'available'>[] = [
...
const OPENROUTER_MODELS: Omit<ModelDef, 'available'>[] = [
  // ── Smart routing ──
...
function formatCost(mult: number): string {
  if (mult === 0) return 'Free'
  if (mult >= 1) return `${mult}x`
  if (mult >= 0.1) return `${mult}x`
  return `${mult}x`
}

function costColor(mult: number): string {
  if (mult === 0)  return 'text-sage-400/70 bg-sage-400/10'
  if (mult <= 0.3) return 'text-sage-400/70 bg-sage-400/10'
  if (mult <= 1)   return 'text-gold-400/70 bg-gold-400/10'
  if (mult <= 3)   return 'text-terra-300/70 bg-terra-300/10'
...
interface Props {
  ...
  connectedProviders?: string[]  // e.g. ['openai', 'copilot', 'gemini']
  compact?: boolean              // minimal mode for inline-input use
  ollamaModels?: OllamaModelInfo[]  // locally installed Ollama models
  gatewayCatalog?: GatewayCatalogEntry[]  // dynamic catalog from models.list
}
...
export const ModelSelector: React.FC<Props> = ({
  ...
  const ollamaModelDefs: ModelDef[] = useMemo(() =>
    ollamaModels.map(m => {
  ...
  // Convert gateway catalog entries to ModelDef format
  const gatewayModelDefs: ModelDef[] = useMemo(() => {
    ...
    return gatewayCatalog.map(m => {
      ...
      return {
        ...
        costMultiplier: 1,
        tier: (m.reasoning ? 'premium' : 'standard') as ModelDef['tier'],
        available: true,  // Gateway only returns models that are actually available
  ...
  const allModels: ModelDef[] = useMemo(() => {
    ...
    // If we have a dynamic gateway catalog, prefer it over hardcoded models
    if (gatewayModelDefs.length > 0) {
      return [auto, ...gatewayModelDefs, ...ollamaModelDefs]
    }

    // Fallback: use hardcoded catalogs with provider availability
    const withAvailability = (models: Omit<ModelDef, 'available'>[], providerKey: string): ModelDef[] =>
      models.map(m => ({ ...m, available: connected.has(providerKey) }))
  ...
  }, [connectedProviders, ollamaModelDefs, gatewayModelDefs])
  ...
  const grouped = useMemo(() => {
    ...
    const groups: { provider: string; icon: string; models: ModelDef[] }[] = []
    ...
    return (
      <div ref={ref} className="relative">
        <button
          ...
          className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg hover:bg-white/[0.06] text-[13px] text-white/50 hover:text-white/80 transition-all"
        >
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-[12px] text-white/60 hover:text-white/90 transition-all"
      >
        {PROVIDER_LOGOS[current.provider]
          ? React.createElement(PROVIDER_LOGOS[current.provider], { size: 11, className: 'text-white/30' })
          : <span className="text-white/30 text-[11px]">{current.providerIcon}</span>
...
const ModelDropdown: React.FC<{
  grouped: { provider: string; icon: string; models: ModelDef[] }[]
  value: string
  search: string
  ...
  alignRight?: boolean
}> = ({ grouped, value, search, onSearch, searchRef, onSelect, alignRight }) => (
  <div className={`absolute bottom-full ${alignRight ? 'right-0' : 'left-0'} mb-2 w-[340px] bg-[#141210] border border-white/[0.08] rounded-xl shadow-2xl shadow-black/60 overflow-hidden z-50`}>
    {/* Search */}
    <div className="px-3 pt-3 pb-2">
      <input
        ...
        className="w-full bg-white/[0.04] border border-white/[0.06] rounded-lg px-3 py-1.5 text-[12px] text-white/80 placeholder-white/20 outline-none focus:border-white/[0.12] transition-colors"
      />
    </div>
    ...
    <div className="max-h-[380px] overflow-y-auto scrollbar-thin pb-2">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {m.contextWindow && (
                    <span className="text-[10px] text-white/15 font-mono">{m.contextWindow}</span>
                  )}
                </div>
              ...
              {/* Selected indicator */}
              {m.id === value && (
                <Check size={13} className="text-terra-400 flex-shrink-0" />
              )}
            </button>
          ))}
        </div>
      ...
      {grouped.length === 0 && (
        <p className="text-center text-[12px] text-white/20 py-6">No models found</p>
      )}
    </div>
  </div>
)
