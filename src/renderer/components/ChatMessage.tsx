...
import {
  Copy, Check, GitBranch, Code2,
  ChevronDown, ChevronUp, ChevronRight,
  Brain, Pencil, Search, Terminal, Loader2,
  FileCode, Play, Eye,
} from 'lucide-react'
...
interface ThinkingBlock { type: 'thinking'; content: string; collapsed: boolean }
interface TextBlock     { type: 'text'; content: string }
interface CodeBlock     { type: 'code'; lang: string; filename?: string; content: string }
interface ToolUseBlock  { type: 'tool-use'; tool: string; status: string; detail?: string }

type ContentBlock = ThinkingBlock | TextBlock | CodeBlock | ToolUseBlock

// ── Component ────────────────────────────────────────────────────────────────

export const ChatMessageBubble: React.FC<Props> = ({
  message, isStreaming, streamingPhase, onBranch
}) => {
    ...
    return (
      <div className="group relative py-4 px-6">
        <div className="max-w-[720px] mx-auto">
          ...
          {message.attachments && message.attachments.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-2">
              {message.attachments.map((a, i) => (
                <div key={i} className="text-[11px] px-2.5 py-1 rounded-lg bg-white/[0.06] text-white/50 border border-white/[0.06] flex items-center gap-1.5">
          ...
          <div className="text-[15px] leading-[1.7] text-white/90 whitespace-pre-wrap break-words"
  ...
  // ── Assistant message ─────────────────────────────────────────────────────
  return (
    <div className="group relative py-4 px-6 bg-white/[0.015]">
      <div className={`max-w-[720px] mx-auto ${isLong && !expanded ? 'max-h-[400px] overflow-hidden relative' : ''}`}>
        {/* Block-based rendering */}
        <div className="flex flex-col">
          {blocks.map((block, i) => (
            <BlockRenderer key={i} block={block} isStreaming={isStreaming && i === blocks.length - 1} />
          ))}

          {/* Streaming cursor */}
          {isStreaming && streamingPhase === 'generating' && (
            <span className="inline-block w-[2px] h-[1.15em] bg-terra-400/80 ml-1 animate-pulse align-middle rounded-full" />
          )}
        </div>

        {/* Fade-out for collapsed long messages */}
        {isLong && !expanded && (
          <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-[#0d0c0a] to-transparent pointer-events-none" />
        )}
      </div>
      ...
      {!isStreaming && message.content && (
        <div className="max-w-[720px] mx-auto flex items-center gap-3 mt-2.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={copy}
            className="flex items-center gap-1 text-[11px] text-white/20 hover:text-white/50 transition-colors">
            {copied ? <Check size={11} className="text-sage-400" /> : <Copy size={11} />}
            {copied ? 'Copied' : 'Copy'}
          ...
          {codeBlockCount > 0 && (
            <span className="flex items-center gap-1 text-[11px] text-terra-300/40">
              <Code2 size={11} /> {codeBlockCount} block{codeBlockCount > 1 ? 's' : ''}
            </span>
          )}
          {onBranch && (
            <button onClick={onBranch}
              className="flex items-center gap-1 text-[11px] text-white/20 hover:text-gold-400 transition-colors">
...
const BlockRenderer: React.FC<{ block: ContentBlock; isStreaming?: boolean }> = ({ block, isStreaming }) => {
  switch (block.type) {
...
const ThinkingBlockView: React.FC<{ block: ThinkingBlock; isStreaming?: boolean }> = ({ block, isStreaming }) => {
  ...
  return (
    <div className="my-2 rounded-xl border border-white/[0.05] bg-white/[0.015] overflow-hidden">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-2.5 w-full px-4 py-2.5 text-left hover:bg-white/[0.02] transition-colors"
      >
        {isStreaming ? (
          <Loader2 size={13} className="animate-spin text-gold-400 flex-shrink-0" />
        ) : (
          <ChevronRight size={13} className={`text-white/25 flex-shrink-0 transition-transform duration-150 ${open ? 'rotate-90' : ''}`} />
        )}
        ...
        {isStreaming && (
          <span className="text-[11px] text-white/20 font-mono tabular-nums ml-1">{formatTime(elapsed)}</span>
        )}
        {!isStreaming && block.content.length > 0 && (
          <span className="text-[11px] text-white/15 ml-1">({Math.ceil(block.content.length / 4)} tokens)</span>
        )}
      </button>
      {open && (
        <div className="px-4 pb-3.5 border-t border-white/[0.04]">
          <div
            className="text-[13px] text-white/30 leading-[1.7] whitespace-pre-wrap mt-2.5 font-mono max-h-[320px] overflow-y-auto scrollbar-thin"
...
const CodeBlockView: React.FC<{ block: CodeBlock }> = ({ block }) => {
  const [copied, setCopied] = useState(false)
  ...
  return (
    <div className="my-2.5 rounded-xl border border-white/[0.06] overflow-hidden bg-[#0a0908]">
      {/* Header bar */}
      <div className="flex items-center justify-between px-4 py-2 bg-white/[0.025] border-b border-white/[0.05]">
        <div className="flex items-center gap-2">
          ...
          {block.lang && (
            <span className="text-[11px] font-mono text-white/30">{block.lang}</span>
          )}
          {block.filename && (
            <span className="text-[11px] font-mono text-terra-300/50">{block.filename}</span>
          )}
        </div>
        <button
          onClick={copyCode}
          className="flex items-center gap-1.5 text-[11px] text-white/25 hover:text-white/60 transition-colors px-2 py-0.5 rounded-md hover:bg-white/[0.05]"
        >
...
const ToolUseBlockView: React.FC<{ block: ToolUseBlock }> = ({ block }) => {
  ...
  return (
    <div className="flex items-center gap-2.5 my-1.5 py-2 px-3.5 rounded-lg bg-white/[0.02] border border-white/[0.04]">
      {isActive ? (
        <Loader2 size={12} className="animate-spin text-gold-400" />
      ) : icon}
      <span className="text-[13px] text-white/40 font-medium">{block.tool}</span>
      {block.detail && (
        <span className="text-[12px] text-white/20 font-mono truncate max-w-[400px]">{block.detail}</span>
      )}
...
const TextBlockView: React.FC<{ block: TextBlock; isStreaming?: boolean }> = ({ block, isStreaming }) => {
  if (!block.content.trim()) return null
  return (
    <div className="py-1">
      <div
        className="text-[15px] whitespace-pre-wrap break-words leading-[1.7] text-white/85 [&_h1]:text-[17px] [&_h1]:font-semibold [&_h1]:text-white/95 [&_h1]:mt-4 [&_h1]:mb-2 [&_h2]:text-[16px] [&_h2]:font-semibold [&_h2]:text-white/90 [&_h2]:mt-3 [&_h2]:mb-1.5 [&_h3]:text-[15px] [&_h3]:font-semibold [&_h3]:text-white/90 [&_h3]:mt-2 [&_h3]:mb-1"
        dangerouslySetInnerHTML={{ __html: renderMarkdown(block.content) }}
      />
      {isStreaming && (
        <span className="inline-block w-[2px] h-[1.15em] bg-terra-400/80 ml-0.5 animate-pulse align-middle rounded-full" />
      )}
    </div>
  )
}
...
function parseContent(raw: string): ContentBlock[] {
  ...
  while ((match = PATTERN.exec(raw)) !== null) {
    if (match.index > lastIndex) {
      ...
      if (text.trim()) pushTextWithToolParsing(blocks, text)
    }
...
function pushTextWithToolParsing(blocks: ContentBlock[], text: string): void {
...
function renderMarkdown(text: string): string {
  ...
  // Inline code
  html = html.replace(/`([^`\n]+)`/g,
    '<code style="background:rgba(255,255,255,0.06);padding:1px 6px;border-radius:4px;font-size:0.88em;font-family:\'JetBrains Mono\',monospace;color:#E5A88A">$1</code>')
  ...
  html = html.replace(/^&gt;\s+(.+)$/gm,
    '<blockquote style="border-left:2px solid rgba(212,120,92,0.3);padding-left:12px;font-style:italic;color:rgba(255,255,255,0.45);margin:6px 0">$1</blockquote>')

  // Unordered lists
  html = html.replace(/^[-*]\s+(.+)$/gm,
    '<li style="margin-left:20px;list-style:disc;color:rgba(255,255,255,0.8);margin:2px 0">$1</li>')

  // Ordered lists
  html = html.replace(/^\d+\.\s+(.+)$/gm,
    '<li style="margin-left:20px;list-style:decimal;color:rgba(255,255,255,0.8);margin:2px 0">$1</li>')

  // Links
  html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g,
    '<a href="$2" style="color:#D4785C;text-decoration:underline;text-underline-offset:2px" target="_blank" rel="noopener">$1</a>')
...
const KW_PATTERN: Record<string, RegExp> = {
